...
void Cmd_CommandRemoveAllBut(NuSMVEnv_ptr env, const char ** whitelist)
{
  avl_tree * commandTable = (avl_tree*)NuSMVEnv_get_value(env, ENV_CMD_COMMAND_TABLE);
  array_t *all_cmd = array_alloc(char *, 5);
  avl_generator * gen;
  avl_tree* white = avl_init_table((int (*)(char*, char*))strcmp);
  int i;
  boolean status;

  for (i = 0; whitelist[i] != NULL; i++) {
    avl_insert(white, (char *)whitelist[i], (char *)100);
  }
  {
    char * key;
    char * value;
    avl_foreach_item(commandTable, gen, AVL_FORWARD, &key, &value) {
      array_insert_last(char *, all_cmd, key);
    }
  }
  {
    char *value;
    char *name;
    arrayForEachItem(char *, all_cmd, i, name) {
      if (avl_lookup(white, name, (char**) NULL) == 0) {
        status = (avl_delete(commandTable, &name, &value) != 0);
        if (status) CmdCommandFree(value);
      }
    }
  }
  array_free(all_cmd);
  avl_free_table(white, (void (*)()) 0, (void (*)()) 0);
}
...
variableInterpolationRecur(const NuSMVEnv_ptr env, char *str)
{
  ...
  while (curStrIndex <= strlen(curStr)) {
    ...
    /* detect a $ if not within quotes.
       NOTE: removed extraneous parentheses here [-Wparentheses-equality] */
    if ('$' == c) {
      if (findEndDollar == 1) {
