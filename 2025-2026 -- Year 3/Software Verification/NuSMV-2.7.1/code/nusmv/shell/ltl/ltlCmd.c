...
int CommandCheckLtlSpec(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  while ((c = Utils_getopt(&opt, argc,argv,"hmo:n:p:P:")) != EOF) {
  ...
  if (argc != opt.optind) return UsageCheckLtlSpec(env);
  ...
  if (formula != NIL(char)) {
    ...
    CATCH(errmgr) {
      PropDbAccessor_verify_prop_at_index(prop_dba, prop_no);
    }
    FAIL(errmgr) {
      status = 1;
    }
    ...
    if (Prop_check_type(PropDbAccessor_get_prop_at_index(
                  prop_dba, prop_no), Prop_Ltl) != 0) {
      status = 1;
    }
    else {
      CATCH(errmgr) {
    ...
    CATCH(errmgr) {
      PropDbAccessor_verify_all_type_wrapper(prop_dba, Prop_Ltl);
    }
    FAIL(errmgr) {
      status = 1;
    }
