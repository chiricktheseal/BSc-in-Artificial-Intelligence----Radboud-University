...
int Bmc_CommandBmcSetup(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  Utils_getopt_state opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, (int)argc, (char**)argv, "hf")) != EOF) {
    switch (c) {
...
int Bmc_CommandBmcSimulate(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  Utils_getopt_state opt = Utils_getopt_init();
  while((c = Utils_getopt(&opt, argc,argv,"t:c:hpvrk:")) != EOF){
    switch(c){
    ...
    case 'r':
      ...
      /* NOTE: Uncomment this if adding support for interactive
         mode. remember to add "ai" to the Utils_getopt format
         parameter */
    /* case 'i': */
    ...
    case 'c':
      ...
      strConstr = util_strsav(opt.optarg);
      isconstraint = true;
      time_shift = true;
      break;
    case 't':
    ...
    case 'k':
      {
        ...
        strNumber = util_strsav(opt.optarg);
...
int Bmc_CommandBmcIncSimulate(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  while((c = Utils_getopt(&opt, argc,argv,"t:c:hpvrk:ia")) != EOF){
    switch(c){
...
int Bmc_CommandBmcPickState(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  Utils_getopt_state opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, argc, argv, "hc:s:vrai")) != EOF) {
    switch (c) {
    case 'h':
      retval = UsageBmcPickState(env);
      goto CommandBmcPickState_exit;

    case 'c': str_constr = util_strsav(opt.optarg); break;
    case 's': str_label = util_strsav(opt.optarg); break;
...
int Bmc_CommandBmcSimulateCheckFeasibleConstraints(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  Utils_getopt_state opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, argc, argv, "hc:q")) != EOF) {
    switch (c) {
    case 'h':
    ...
    case 'c': {
      ...
      char* str_constr = util_strsav(opt.optarg);
...
static int UsageBmcCheckInvar(const NuSMVEnv_ptr env)
{
  ...
  StreamMgr_print_error(streams,  "\t\tValid values are: "
          BMC_INVAR_ALG_CLASSIC " (aka k-induction with k=1), " BMC_INVAR_ALG_EEN_SORENSSON
...
static int bmc_CommandCheckPslSpecBmc(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  const ErrorMgr_ptr errmgr =
    ERROR_MGR(NuSMVEnv_get_value(env, ENV_ERROR_MANAGER));
  PropDbAccessor_ptr const prop_dba =
      PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA));
  OptsHandler_ptr const opts =
     OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));
  ...
  Utils_getopt_state opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, argc, argv, "h1gmo:n:p:P:k:l:")) != EOF) {
    switch (c) {
    case 'h': {
    ...
    case 'n':
      {
        ...
        prop_no = PropShell_get_prop_index_from_string(prop_dba,
                                                    opt.optarg);
        if (prop_no == -1) {
    ...
    case 'P':
      {
        ...
        formula_name = util_strsav(opt.optarg);

        prop_no = PropDbAccessor_prop_parse_name(prop_dba,
                                         formula_name);
    ...
    case 'l':
      ...
      str_loop = util_strsav(opt.optarg);
      l_specified = true;
    ...
    case 'p':
      {
        ...
        formula = util_strsav(opt.optarg);
        break;
      }
    case 'o':
      if (useMore) {
      ...
      dbgFileName = util_strsav(opt.optarg);
  ...
  if (formula != NIL(char)) {
    ...
    prop_no = PropDbAccessor_prop_parse_and_add(prop_dba, st,
...
int Bmc_CommandProfile(NuSMVEnv_ptr env, int argc, char ** argv)
{
  ...
  boolean disable;
  char* out_file_name;
  Utils_getopt_state opt;

  enable = false;
  disable = false;
  out_file_name = (char*) NULL;

  opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, (int) argc, (char**) argv, "hde:")) != EOF) {
    switch (c) {
    ...
    case 'e':
      ...
      enable = true;
      out_file_name = util_strsav(opt.optarg);
      break;
...
int Bmc_CommandWatchdog(NuSMVEnv_ptr env, int argc, char ** argv)
{
  ...
  timeout = -1;
  while ((c = Utils_getopt(&opt, (int) argc, (char**) argv, "he:t:")) != EOF) {
    switch (c) {
    ...
    case 'e':
      {
        ...
        char* err_occ[2];
        str_number = util_strsav(opt.optarg);
...
int Bmc_TestTableau(NuSMVEnv_ptr env, int argc, char ** argv)
{
  ...
  /* process command options */
  {
    ...
    Utils_getopt_state opt = Utils_getopt_init();
    while((c = Utils_getopt(&opt, argc, argv, "hn:d:c:po:x")) != EOF) {
      switch (c) {
      case 'n':
        prop_no = PropShell_get_prop_index_from_string(prop_dba, opt.optarg);
        if (-1 == prop_no)
          return 1;

        ltlprop = PropDbAccessor_get_prop_at_index(prop_dba, prop_no);
        nusmv_assert(NULL != ltlprop);
      ...
      case 'o': /* operator specification */
        szOperator = util_strsav(opt.optarg);
                         ...
                         int *res_step_k)
{
  ...
  /* ---------------------------------------------------------------------- */
  /* Fills up the string to pass to Utils_getopt, depending on which
  ...
  opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, (int)argc, (char**) argv, opt_string)) != EOF) {
    switch (c) {
    ...
    case 'n':
      {
        ...
        str_prop_idx = util_strsav(opt.optarg);

        /* check if property idx is ok */
        prop_idx = PropShell_get_prop_index_from_string(
                                    prop_dba, str_prop_idx);
        FREE(str_prop_idx);
        ...
        *res_prop = PropDbAccessor_get_prop_at_index(prop_dba,
                                             prop_idx);
    ...
    case 'P':
      {
        ...
        prop_idx = PropDbAccessor_prop_parse_name(prop_dba,
                                          formula_name);
    ...
    case 'p':
      ...
      str_formula = util_strsav(opt.optarg);
      break;
    ...
    case 's':
      ...
      *res_s = util_strsav(opt.optarg);
      break;

    case 'o':
      nusmv_assert(res_o != (char**) NULL);

      *res_o = util_strsav(opt.optarg);
      break;
  ...
  if (str_formula != (char*) NULL) {
    ...
    idx = PropDbAccessor_prop_parse_and_add(prop_dba,
                                    symb_table,
                                            ...
                                            const boolean has_to_solve)
{
  ...
  if (ltlprop == PROP(NULL)) {
    const PropDbAccessor_ptr prop_dba = PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA));
    lsList props;
    lsGen  iterator;
    Prop_ptr prop;

    props = PropDbAccessor_prepare_prop_list(prop_dba, Prop_Ltl);
...
{
  if (ltlprop == PROP(NULL)) {
    PropDbAccessor_ptr prop_dba = PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA));
    lsList props;
    lsGen  iterator;
    Prop_ptr prop;
                                                  ...
                                                  const boolean has_to_solve)
{
  ...
  if (invarprop == PROP(NULL)) {
    ...
    props = PropDbAccessor_prepare_prop_list(prop_dba, Prop_Invar);
...
bmc_cmd_gen_solve_psl_selected_or_all_props(NuSMVEnv_ptr const env,
                                            ...
                                            const int l)
{
  const PropDbAccessor_ptr prop_dba =
      PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA));
  int status = 0;

  if (prop_no != -1) {
    if (Prop_check_type(PropDbAccessor_get_prop_at_index(prop_dba,
                                                 prop_no),
                        Prop_Psl) != 0) {
      status = 1;
    }
      ...
      status =
        Bmc_Gen_check_psl_property(env,
                                   PropDbAccessor_get_prop_at_index(prop_dba, prop_no),
                                   bmc_dump, inc_sat,
                                   single_bmc_prob, k, l);
    }
  }
    ...
    props = PropDbAccessor_prepare_prop_list(prop_dba, Prop_Psl);
