...
static int sbmc_CommandCheckPslSpecSbmc(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  PropDbAccessor_ptr const prop_dba = PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA));
...
static int sbmc_CommandCheckPslSpecSbmcInc(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  /* It is possible to extend sbmc_cmd_options_handling in order to handle the
     extra options of this function: g m */
  Utils_getopt_state opt = Utils_getopt_init();
  while ((c = Utils_getopt(&opt, argc, argv, "hicN1gmo:n:p:P:k:l:")) != EOF) {
    switch (c) {
    case 'h': {
      status = BMC_USAGE; goto label_clean_and_exit;
    }
                          ...
                          boolean* res_1)
{
  ...
  while ((c = Utils_getopt(&opt, (int)argc, (char**) argv, opt_string)) != EOF) {
    ...
    case 'n': {
      ...
      prop_idx = PropShell_get_prop_index_from_string(PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA)),
                                                   str_prop_idx);
      FREE(str_prop_idx);
      ...
      *res_prop = PropDbAccessor_get_prop_at_index(PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA)),
                                           prop_idx);
    ...
    case 'P':
      {
        ...
        prop_idx = PropDbAccessor_prop_parse_name(PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA)),
                                          formula_name);
  ...
  if (str_formula != (char*) NULL) {
    ...
    idx = PropDbAccessor_prop_parse_and_add(PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA)), st,
    ...
    *res_prop = PropDbAccessor_get_prop_at_index(PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA)), idx);
                                                    ...
                                                    const boolean do_completeness_check)
{
  if (ltlprop == PROP(NULL)) {
    ...
    lsGen  iterator;
    Prop_ptr prop;
    lsList const props = PropDbAccessor_prepare_prop_list(prop_dba, Prop_Ltl);
