...
boolean psl_node_sere_is_trans_propositional(PslNode_ptr e)
{
  if (e==NULL) return false;
  if (psl_node_get_op(e)!=PSL_SERE) return false;
  if (PslNode_is_trans_propositional(psl_node_get_left(e))) return true;
  return psl_node_sere_is_trans_propositional(psl_node_get_left(e));
}
                                         ...
                                         boolean accept_next)
{
  ...
  case CONTEXT:
    result = psl_node_is_propositional(psl_node_get_right(expr), accept_next);
    break;
  ...
  case SMALLINIT:
    result = psl_node_is_propositional(psl_node_get_left(expr), false);
    break;
  case NOT:
  case PSL_TILDE:
    result = psl_node_is_propositional(psl_node_get_left(expr), accept_next);
    break;
  ...
  case IMPLIES:
    result = psl_node_is_propositional(psl_node_get_left(expr), accept_next) &&
      psl_node_is_propositional(psl_node_get_right(expr), accept_next);
    break;

  case CASE:
    result = psl_node_is_propositional(psl_node_get_case_cond(expr), accept_next) &&
      psl_node_is_propositional(psl_node_get_case_then(expr), accept_next) &&
      psl_node_is_propositional(psl_node_get_case_next(expr), accept_next);
    break;

  case PSL_ITE:
    result = psl_node_is_propositional(psl_node_get_ite_cond(expr), accept_next) &&
      psl_node_is_propositional(psl_node_get_ite_then(expr), accept_next) &&
      psl_node_is_propositional(psl_node_get_ite_else(expr), accept_next);
    break;

  case PSL_REPLPROP:
    result = psl_node_is_propositional(psl_node_repl_prop_get_property(expr), accept_next);
    break;
