...
{
  /* tells if finalizer should be called or not */
  VIRTUAL boolean (*pre_finalizer)(Object_ptr self, void* arg);
...
/*!
  ...
  Must be called by derived class inizializer *after* any other
  operation. The deinizializer in derived class must be called only by
  the finalizer (which is called the destructor, if
  pre_finalizer says so). No other operation is
  allowed on the instance is being to be destroyed.
