...
static Object_ptr env_object_copy(const Object_ptr self);
...
void env_object_init(EnvObject_ptr self, NuSMVEnv_ptr env)
{
  ...
  OVERRIDE(Object, copy) = env_object_copy;
}
...
/*!
  ...
  \sa env_object_copy_aux
*/
static Object_ptr env_object_copy(const Object_ptr self)
{
  EnvObject_ptr other = ALLOC(EnvObject, 1);
  ENV_OBJECT_CHECK_INSTANCE(other);

  env_object_copy_aux(ENV_OBJECT(self), other);
  return OBJECT(other);
}
