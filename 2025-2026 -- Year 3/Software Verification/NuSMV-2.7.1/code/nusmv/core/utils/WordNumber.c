...
char* WordNumber_to_based_string(WordNumber_ptr self, int base, boolean isSigned)
{
  ...
  return word_number_internal_value_to_based_string_aux(self->value,
                                                        self->width,
                                                        base,
                                                        isSigned);
}
...
char *
word_number_internal_value_to_based_string_aux(WordNumberValue value,
                                               int width,
                                               int base,
                                               boolean isSigned)
{
  ...
  case 2: {
    int n = width - 1;
    WordNumberValue v = value;
    char* str = buffer;

    printed = snprintf(str, buffer_size, "0%sb%i_", isSigned ? "s" : "u", width);
  ...
  case 10: {
    ...
    if (isSigned) { /* the word is signed => check the sign */
      ...
      if (sign) { /* get positive number */
        value = -((((~0ULL) >> (width-1)) << (width-1)) | value);
      }
    }
