...
*/
#define SSET_GET_BALANCE(node)    (PTR_TO_PTRINT((node)->parent) & 0x3)
...
#define SSET_SET_BALANCE(node, balance) \
  { DEBUG(nusmv_assert((balance & ~0x3) == 0));                      \
    ((node)->parent =                                                \
        PTR_FROM_INT(void*, (PTR_TO_PTRINT((node)->parent) & ~0x3) | \
                     (balance)));                                    \
  }
...
#define SSET_INC_BALANCE(node) \
  ((node)->parent =                                                           \
      PTR_FROM_INT(void*, (((PTR_TO_PTRINT((node)->parent) & 0x3) + 1) & 0x3) \
                          | (PTR_TO_PTRINT((node)->parent) & ~0x3))           \
  )
...
/*!
  ...
  See the description of struct Ssnode_TAG for more info.
*/
#define SSET_GET_PARENT(node) PTR_FROM_INT(void*, \
                                           PTR_TO_PTRINT((node)->parent) & ~0x3)
...
#define SSET_SET_PARENT(node, _parent) \
  { DEBUG(nusmv_assert((PTR_TO_PTRINT(_parent) & 0x3) == 0));                  \
    (node)->parent = PTR_FROM_INT(void*, (PTR_TO_PTRINT((node)->parent) & 0x3) \
                                         | PTR_TO_PTRINT(_parent));            \
  }
...
#define SSET_SET_PARENT_BALANCE(node, _parent, balance) \
  { DEBUG(nusmv_assert((PTR_TO_PTRINT(_parent) & 0x3) == 0                      \
                        && ((balance) & ~0x3) == 0));                           \
    ((node)->parent = PTR_FROM_INT(void*, PTR_TO_PTRINT(_parent) | (balance))); \
  }
