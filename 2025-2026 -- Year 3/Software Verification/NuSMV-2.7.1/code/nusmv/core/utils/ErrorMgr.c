...
void ErrorMgr_start_parsing_err(ErrorMgr_ptr self)
{
  ...
  if (nusmv_yylineno > 0)
    StreamMgr_print_error(self->streams, "line %d: ", nusmv_yylineno);
}
...
void ErrorMgr_error_not_constant_number_expression(ErrorMgr_ptr self,
                                                   node_ptr name)
{
  FILE* err;
  NuSMVEnv_ptr env;
  ...
  ErrorMgr_start_parsing_err(self);
  StreamMgr_print_error(self->streams, "the expression ");
  print_node(wffprint, err, name);
  StreamMgr_print_error(self->streams, " is not a constant number expression");
  ErrorMgr_finish_parsing_err(self);
}
