...
static void bdd_fsm_update_cache(BddFsm_ptr self);

static void
bdd_fsm_apply_synchronous_product_custom_varsets (BddFsm_ptr self,
...
/* Cache substitution. The hard copy is necessary whenever any essential
   component (init, invars, trans, justice, compassion) of the BddFsm is
   changed. Code is from BddFsm_apply_synchronous_product_custom_varsets()
 */
static void bdd_fsm_update_cache(BddFsm_ptr self)
{
...
void BddFsm_set_justice(BddFsm_ptr self, JusticeList_ptr new_justice)
{
  BDD_FSM_CHECK_INSTANCE(self);

  if ((JusticeList_ptr) NULL != self->justice) {
    Object_destroy(OBJECT(self->justice), NULL);
  }
  self->justice = new_justice;

  /* cache substitution */
  bdd_fsm_update_cache(self);
}
...
void BddFsm_set_compassion(BddFsm_ptr self, CompassionList_ptr new_compassion)
{
  BDD_FSM_CHECK_INSTANCE(self);

  if ((CompassionList_ptr) NULL != self->compassion) {
    Object_destroy(OBJECT(self->compassion), NULL);
  }
  self->compassion = new_compassion;
...
void BddFsm_set_init(BddFsm_ptr self, BddStates new_init)
{
  bdd_ptr res;

  BDD_FSM_CHECK_INSTANCE(self);

  res = bdd_dup((bdd_ptr) new_init);
  if ((bdd_ptr) NULL != self->init) {
...
void BddFsm_set_state_constraints(BddFsm_ptr self, BddInvarStates new_invar_states)
{
  bdd_ptr res;

  BDD_FSM_CHECK_INSTANCE(self);

  res = bdd_dup((bdd_ptr) new_invar_states);
  if ((bdd_ptr) NULL != self->invar_states) {
    bdd_free(self->dd, (bdd_ptr) self->invar_states);
  }
  self->invar_states = res;
...
void  BddFsm_set_input_constraints(BddFsm_ptr self, BddInvarInputs new_invar_inputs)
{
  bdd_ptr res;

  BDD_FSM_CHECK_INSTANCE(self);

  res = bdd_dup((bdd_ptr) new_invar_inputs);
  if ((bdd_ptr) NULL != self->invar_inputs) {
    bdd_free(self->dd, (bdd_ptr) self->invar_inputs);
  }
  self->invar_inputs = res;
...
void BddFsm_set_trans(BddFsm_ptr self, BddTrans_ptr new_trans)
{
  BDD_FSM_CHECK_INSTANCE(self);

  if ((BddTrans_ptr) NULL != self->trans) {
                                     ...
                                     BddStatesInputsNexts constraints)
{
  ...
  {
    bdd_ptr iv = BddEnc_get_input_vars_cube(self->enc);
    bdd_ptr t1 = bdd_forsome(self->dd, res, iv);
    bdd_free(self->dd, res);
    res = t1;
    bdd_free(self->dd, iv);
  }
                                      ...
                                      BddStatesInputsNexts constraints)
{
  ...
  {
    bdd_ptr iv = BddEnc_get_input_vars_cube(self->enc);
    bdd_ptr t1 = bdd_forsome(self->dd, result, iv);
                                             ...
                                             BddStates next_states)
{
  ...
  {
    bdd_ptr nv = BddEnc_get_next_state_vars_cube(self->enc);
    bdd_ptr cv = BddEnc_get_state_vars_cube(self->enc);
    bdd_ptr t1 = bdd_forsome(self->dd, inputs, nv);
    bdd_free(self->dd, inputs);
    inputs = bdd_forsome(self->dd, t1, cv);
    bdd_free(self->dd, nv);
    bdd_free(self->dd, cv);
    bdd_free(self->dd, t1);
  }
                                                  ...
                                                  boolean init_p)
{
  ...
  /* invars (states and inputs) */
  bdd_and_accumulate(self->dd, &(self->invar_states), other->invar_states);
  bdd_and_accumulate(self->dd, &(self->invar_inputs), other->invar_inputs);
...
void BddFsm_apply_synchronous_product_custom_varsets(BddFsm_ptr self,
                                                     ...
                                                     bdd_ptr next_vars_cube)
{
  bdd_fsm_apply_synchronous_product_custom_varsets
    (self, other, state_vars_cube, input_vars_cube, next_vars_cube, true);
}

void BddFsm_apply_synchronous_product_extended (BddFsm_ptr self, const BddFsm_ptr other,
                                                boolean init_p)
{
  bdd_ptr input_vars_cube;
  bdd_ptr state_vars_cube;
  ...
  bdd_fsm_apply_synchronous_product_custom_varsets
    (self, other, state_vars_cube, input_vars_cube, next_vars_cube, init_p);
...
{
  BddFsm_apply_synchronous_product_extended(self, other, true);
}
