                                    ...
                                    hash_ptr mod_defs)
{
  ...
  for (decls = mod_body; Nil != decls; decls = cdr(decls)) {
    ...
    case URGENT:
      HrcNode_add_urgent_expr(hrc_result, car(cur_decl));
      break;
      
    case SPEC:
      HrcNode_add_ctl_property_expr(hrc_result, car(cur_decl));
      break;

    case LTLSPEC:
      HrcNode_add_ltl_property_expr(hrc_result, car(cur_decl));
      break;

    case PSLSPEC:
      HrcNode_add_psl_property_expr(hrc_result, car(cur_decl));
      break;

    case INVARSPEC:
      HrcNode_add_invar_property_expr(hrc_result, car(cur_decl));
      break;

    case COMPUTE:
      HrcNode_add_compute_property_expr(hrc_result, car(cur_decl));
      break;
    ...
    case COMPASSION:
      HrcNode_add_compassion_expr(
          hrc_result,
          NodeMgr_cons(nomgr, caar(cur_decl), cdar(cur_decl)));
      break;

    /* ---------------------------------------------------------------------
     * These cases are to be moved into nuxmv after refactoring of
     * compiler and node traversals
     * --------------------------------------------------------------------- */

    /* ignored cases:  */
    case PRED:
    case MIRROR:
      /* no action here for the moment */
      break;

    case PSYNTH_PROB:
      HrcNode_add_psynth_problem(hrc_result, cur_decl);
      break;

    /* ---------------------------------------------------------------------
     * End of cases to be moved to nuxmv
     * --------------------------------------------------------------------- */
