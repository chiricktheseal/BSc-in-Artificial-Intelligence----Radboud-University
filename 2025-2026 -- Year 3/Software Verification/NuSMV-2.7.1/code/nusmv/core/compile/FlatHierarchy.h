...
/*!
  ...
  2. a variable name to all constraints (INIT, TRANS, INVAR, URGENT) which constain
  the given variable.
...
/*!
  ...
  These class stores:
  the list of TRANS, INIT, INVAR, ASSIGN, URGENT, SPEC, COMPUTE, LTLSPEC,
  ...
  5. The association var->constraints (init, trans, invar, urgent) should be
  for constraints of this hierarchy only. Similar to
  var->assignment association (see above) a variable may
  potentially be from another hierarchy.
  See FlatHierarchy_lookup_constraints, FlatHierarchy_add_constraints
...
/*!
  ...
  Allows to not specify urgent explicitly.
  \se FlatHierarchy_create_from_members_timed
*/
FlatHierarchy_ptr
...
/* Access function to the class's fields : constraints and specifications ******/
...
*/
void
FlatHierarchy_set_assign(FlatHierarchy_ptr self, node_ptr n);
...
/*!
  ...
  It is a cons list of constraintts
*/
...
*/
void
FlatHierarchy_set_justice(FlatHierarchy_ptr self, node_ptr n);
...
/*!
  ...
  \todo Missing description
*/
void FlatHierarchy_set_compassion(FlatHierarchy_ptr self,
                                  node_ptr n);
...
*/
void
FlatHierarchy_set_ltlspec(FlatHierarchy_ptr self, node_ptr n);
...
/*!
  ...
  \todo Missing description
*/
void FlatHierarchy_set_invarspec(FlatHierarchy_ptr self,
                                 node_ptr n);
...
*/
void
FlatHierarchy_set_pslspec(FlatHierarchy_ptr self, node_ptr n);
...
*/
void
FlatHierarchy_set_compute(FlatHierarchy_ptr self, node_ptr n);
...
/*!
  \brief Returns the list (cons) of previously set or add param synth
  problems
*/
node_ptr FlatHierarchy_get_psynth_problems(const FlatHierarchy_ptr self);

/*!
  \brief Adds a single param synth problems to the internally hold list
*/
void FlatHierarchy_add_psynth_problem(
    FlatHierarchy_ptr self, node_ptr n);

/*!
  \brief Sets the list (cons) of param synth problems
*/
void FlatHierarchy_set_psynth_problems(
    FlatHierarchy_ptr self, node_ptr n);

/*!
  \brief Returns the list (cons) of previously set or add KLTL problems
*/
node_ptr FlatHierarchy_get_kltl_problems(const FlatHierarchy_ptr self);

/*!
  \brief Adds a single KLTL problem to the internally hold list
*/
void FlatHierarchy_add_kltl_problem(FlatHierarchy_ptr self, node_ptr n);

/*!
  \brief Sets the list (cons) of KLTL problems
*/
...
/* Access function to the hash.
   ...
   The functions also can return the list of INIT, INVAR, TRANS and URGENT expressions
...
/*!
  \methodof FlatHierarchy
  \brief Returns  a list of constraints which contain
  ...
  If the parameter "name" has a form urgent(var-name) then
  the URGENT expressions are returned.
  ...
  \sa FlatHierarchy_add_constraints
*/
...
/*!
  ...
  of constraints associated to the given variable
  ...
  \sa FlatHierarchy_lookup_constraints
*/
void FlatHierarchy_add_constraints(FlatHierarchy_ptr self,
                                  node_ptr name,
                                  node_ptr expr);
/*
 !
  ...
  Like FlatHierarchy_add_constraints but allows to specify the expr type:
  thus the constraint is added using the proper operator;
  FlatHierarchy_add_constraints adds the new constraint by conjunction
  NB: All given expressions should have been declared in the given hierarchy.
*/
void FlatHierarchy_add_constraints_type(FlatHierarchy_ptr self,
                                       node_ptr name,
                                       node_ptr expr,
                                       const int type);
/*!
  \methodof FlatHierarchy
  \brief Retrieves the list of constraints associated to constants

  Retrieves the list of constraints associated to constants
  for the given hierarchy section.
  Type must be one of "INIT, INVAR, TRANS or URGENT"

  \sa FlatHierarchy_add_constant_constraints
*/
node_ptr
FlatHierarchy_lookup_constant_constraints(FlatHierarchy_ptr self,
                                         int type);

/*!
  \methodof FlatHierarchy
  \brief Adds the given expressions to the list
  of constraints associated to constants

  Adds the given expressions to the list
  of constraints associated to constants.
  Type must be one of "INIT, INVAR, TRANS or URGENT"

  \sa FlatHierarchy_lookup_constant_constraints
*/
...
/*!
  ...
  4. urgent(var-name) and URGENT expression list.
...
*/
void
FlatHierarchy_calculate_vars_constraints(FlatHierarchy_ptr self);
...
*/
hash_ptr
FlatHierarchy_get_var_expr_associations(const FlatHierarchy_ptr self);
...
/*!
  ...
  FlatHierarchy_insert_assign and FlatHierarchy_add_constraints such that
  functions FlatHierarchy_lookup_assign and FlatHierarchy_lookup_constraints
...
*/
hash_ptr
FlatHierarchy_get_constants_associations(const FlatHierarchy_ptr self);
