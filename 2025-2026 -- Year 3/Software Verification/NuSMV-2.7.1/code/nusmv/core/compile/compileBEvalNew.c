                                       ...
                                       boolean in_next, hash_ptr expr2bexpr)
{
  ...
  {  /* not a leaf, applies the standard encoding */
    ...
    return scalar_atom2bexpr(enc, det_layer, expr, context, in_next);
  }
}
                                 ...
                                 hash_ptr expr2bexpr)
{
...

  {
    ...
    case FALSEEXP:
      ...
      break;
    case TIME:
      ErrorMgr_rpterr(errmgr, "time cannot be casted to boolean");
      break;      
    case NUMBER_FRAC:
    case NUMBER_REAL:
    case NUMBER_EXP:
...
static Expr_ptr expr2bexpr_extend(BddEnc_ptr enc, SymbLayer_ptr det_layer,
  ...
  ta = TypeChecker_get_expression_type(BaseEnc_get_type_checker(BASE_ENC(enc)),
                                       car(expr), context);
  nusmv_assert(SymbType_is_word(ta));
