...
static Object_ptr checker_base_copy(const Object_ptr self);
...
static void _checker_base_print_operator(const CheckerBase_ptr checker,
                                         FILE* output_stream,
                                         const node_ptr expr);
...
void checker_base_copy_aux(const CheckerBase_ptr self,
                           CheckerBase_ptr other)
{
  /* base class copy constructor */
  node_walker_copy_aux(NODE_WALKER(self), NODE_WALKER(other));

  /* virtual methods copy */
  other->check_expr = self->check_expr;
  other->viol_handler = self->viol_handler;
  other->print_operator = self->print_operator;
}
...
VIRTUAL void checker_base_print_operator(const CheckerBase_ptr checker,
                                         FILE* output_stream, const node_ptr expr)
{
  checker->print_operator(checker, output_stream, expr);
}
...
static Object_ptr checker_base_copy(const Object_ptr self)
{
  CheckerBase_ptr other = ALLOC(CheckerBase, 1);
  CHECKER_BASE_CHECK_INSTANCE(other);

  checker_base_copy_aux(CHECKER_BASE(self), other);
  return OBJECT(other);
}
                                         ...
                                         FILE* output_stream, const node_ptr expr)
{
  ...
  ErrorMgr_internal_error(errmgr, "CheckerBase: Pure virtual method print_operator " \
                          "not implemented\n");
}
