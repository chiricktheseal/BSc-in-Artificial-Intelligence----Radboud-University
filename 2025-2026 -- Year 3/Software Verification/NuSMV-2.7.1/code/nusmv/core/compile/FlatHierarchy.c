...
{
  SymbTable_ptr st;
  boolean vars_constr_built; /* whether vars constraints have been built */
  NodeMgr_ptr nodemgr;
  ...
  node_ptr psynth_problems;
  node_ptr kltl_problems;
...
static void
flat_hierarchy_self_check_expr(const FlatHierarchy_ptr self, node_ptr expr);
...
/* Wrapper for compatibility with other modules, allows to not specify urgent */
...
{
  return
    FlatHierarchy_create_from_members_timed(st, init, invar, trans, Nil, input,
                                            justice, compassion);
}

FlatHierarchy_ptr
FlatHierarchy_create_from_members_timed(SymbTable_ptr st,
                                  node_ptr init, node_ptr invar,
                                  node_ptr trans, node_ptr urgent, node_ptr input,
                                  node_ptr justice, node_ptr compassion)
{
  ...
  FlatHierarchy_set_urgent(self, urgent);
...
node_ptr FlatHierarchy_get_init(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_invar(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_trans(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_input(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
*/
node_ptr FlatHierarchy_get_assign(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_urgent(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
  return(self->urgent_expr);
}
void FlatHierarchy_set_urgent(FlatHierarchy_ptr self, node_ptr n)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
  /* later recalculates vars constr if needed */
  self->vars_constr_built = self->vars_constr_built && (self->urgent_expr == n);
  self->urgent_expr = n;
}

node_ptr FlatHierarchy_get_justice(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_compassion(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_spec(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_ltlspec(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_invarspec(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_pslspec(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_compute(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
#if !defined(_MSC_VER)
#warning [AMi] NEXT are overwritten! Are we sure that this is a desired feature
#endif
...
node_ptr FlatHierarchy_lookup_constraints(FlatHierarchy_ptr self,
                                         node_ptr name)
{
  node_ptr res;
  NuSMVEnv_ptr env;
  ...
  /* if not previously calculated (or if invalidated by later
     settings) triggers calculation of var constraints */
  if (!self->vars_constr_built) {
    if (opt_verbose_level_gt(opts, 1)) {
      Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
      Logger_log(logger, "Triggering postponed "\
              "calculation of vars constraints\n");
    }
    FlatHierarchy_calculate_vars_constraints(self);
  }
...
void FlatHierarchy_add_constraints_type(FlatHierarchy_ptr self, node_ptr name,
                                       node_ptr expr, const int type)
{
  node_ptr cont;
  const int separator_symbol = (type == URGENT)? OR : AND;
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
  cont = find_assoc(self->assign_hash, name);
  ...
  else setcdr(cont, find_node(self->nodemgr, separator_symbol, cdr(cont), expr));
}

void FlatHierarchy_add_constraints(FlatHierarchy_ptr self, node_ptr name,
                                  node_ptr expr)
{
  node_ptr cont;
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_lookup_constant_constraints(FlatHierarchy_ptr self,
                                                  int type)
{
  node_ptr res;
  NuSMVEnv_ptr env;
  ...
  nusmv_assert(INIT == type || TRANS == type || INVAR == type || URGENT == type);
...
void FlatHierarchy_add_constant_constraints(FlatHierarchy_ptr self,
                                           node_ptr expr,
                                           int type)
{
  node_ptr tmp;
  ...
  nusmv_assert(INIT == type || TRANS == type || INVAR == type || URGENT == type);
  {
    ...
    /* If a constraint already exist, put it in AND with the new one.. */
    if (Nil != tmp) {
      switch(type) {
      case URGENT:
        tmp = ExprMgr_or_nil(exprs, tmp, expr);
        break;
      case INIT: case TRANS: case INVAR:
        tmp = ExprMgr_and(exprs, tmp, expr);
        break;
      default:
...
hash_ptr FlatHierarchy_get_constants_associations(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
void FlatHierarchy_calculate_vars_constraints(FlatHierarchy_ptr self)
{
  NuSMVEnv_ptr env;
  OptsHandler_ptr opts;
  ...
  flat_hierarchy_calc_var_const_aux(self, FlatHierarchy_get_urgent(self), URGENT);

  self->vars_constr_built = true;
  if (opt_verbose_level_gt(opts, 1)) {
    Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
    Logger_log(logger, "FlatHierarchy: vars constraints calculated\n");
  }
}

hash_ptr FlatHierarchy_get_var_expr_associations(const FlatHierarchy_ptr self)
{
  FLAT_HIERARCHY_CHECK_INSTANCE(self);
...
node_ptr FlatHierarchy_get_preds(const FlatHierarchy_ptr self)
{
  return self->pred_list;
}

void FlatHierarchy_add_pred(FlatHierarchy_ptr self, node_ptr n)
{
  self->pred_list = cons(self->nodemgr, n, self->pred_list);
}

void FlatHierarchy_set_pred(FlatHierarchy_ptr self, node_ptr n)
{
  self->pred_list = n;
}

node_ptr FlatHierarchy_get_mirrors(const FlatHierarchy_ptr self)
{
  return self->mirror_list;
}

void FlatHierarchy_add_mirror(FlatHierarchy_ptr self, node_ptr n)
{
  self->mirror_list = cons(self->nodemgr, n, self->mirror_list);
}

void FlatHierarchy_set_mirror(FlatHierarchy_ptr self, node_ptr n)
{
  self->mirror_list = n;
}

node_ptr FlatHierarchy_get_property_patterns(const FlatHierarchy_ptr self)
{
  return self->property_patterns;
}

void FlatHierarchy_add_property_pattern(FlatHierarchy_ptr self, node_ptr n)
{
  self->property_patterns = cons(self->nodemgr, n, self->property_patterns);
}

void FlatHierarchy_set_property_patterns(FlatHierarchy_ptr self, node_ptr n)
{
  self->property_patterns = n;
}

node_ptr FlatHierarchy_get_psynth_problems(const FlatHierarchy_ptr self)
{
  return self->psynth_problems;
}

void FlatHierarchy_add_psynth_problem(FlatHierarchy_ptr self, node_ptr n)
{
  self->psynth_problems = cons(self->nodemgr, n, self->psynth_problems);
}

void FlatHierarchy_set_psynth_problems(FlatHierarchy_ptr self, node_ptr n)
{
  self->psynth_problems = n;
}

node_ptr FlatHierarchy_get_kltl_problems(const FlatHierarchy_ptr self)
{
  return self->kltl_problems;
}

void FlatHierarchy_add_kltl_problem(FlatHierarchy_ptr self, node_ptr n)
{
  self->kltl_problems = cons(self->nodemgr, n, self->kltl_problems);
}

void FlatHierarchy_set_kltl_problems(FlatHierarchy_ptr self, node_ptr n)
{
  self->kltl_problems = n;
}

void FlatHierarchy_self_check(const FlatHierarchy_ptr self)
{
  flat_hierarchy_self_check_expr(self, self->init_expr);
  flat_hierarchy_self_check_expr(self, self->invar_expr);
  flat_hierarchy_self_check_expr(self, self->trans_expr);
  flat_hierarchy_self_check_expr(self, self->input_expr);
  flat_hierarchy_self_check_expr(self, self->assign_expr);
  flat_hierarchy_self_check_expr(self, self->urgent_expr);
  flat_hierarchy_self_check_expr(self, self->justice_expr);
  flat_hierarchy_self_check_expr(self, self->compassion_expr);
  flat_hierarchy_self_check_expr(self, self->spec_expr);
  flat_hierarchy_self_check_expr(self, self->ltlspec_expr);
  flat_hierarchy_self_check_expr(self, self->invarspec_expr);
  flat_hierarchy_self_check_expr(self, self->compute_expr);
}
...
static void flat_hierarchy_init(FlatHierarchy_ptr self, SymbTable_ptr st)
{
  ...
  self->psynth_problems      = Nil;
  self->kltl_problems        = Nil;
...
flat_hierarchy_copy(const FlatHierarchy_ptr self, FlatHierarchy_ptr other)
{
  ...
  other->urgent_expr          = self->urgent_expr;
  ...
  other->psynth_problems      = self->psynth_problems;
  other->kltl_problems        = self->kltl_problems;
                         ...
                         const FlatHierarchy_ptr other)
{
  ...
  self->urgent_expr  = NODE_PTR(ExprMgr_or_nil(exprs, EXPR(self->urgent_expr),
                                                EXPR(other->urgent_expr)));
 ...
 * append simply the module assigments to self */
  ...
  /* Constant constraints merging */
  {
    node_ptr tmp = FlatHierarchy_lookup_constant_constraints(other, INIT);
    FlatHierarchy_add_constant_constraints(self, tmp, INIT);

    tmp = FlatHierarchy_lookup_constant_constraints(other, INVAR);
    FlatHierarchy_add_constant_constraints(self, tmp, INVAR);

    tmp = FlatHierarchy_lookup_constant_constraints(other, TRANS);
    FlatHierarchy_add_constant_constraints(self, tmp, TRANS);

    tmp = FlatHierarchy_lookup_constant_constraints(other, URGENT);
    FlatHierarchy_add_constant_constraints(self, tmp, URGENT);
  }
  ...
  self->psynth_problems = append_ns(nodemgr, self->psynth_problems,
                                    self->psynth_problems);

  self->kltl_problems = append_ns(nodemgr, self->kltl_problems,
                                  self->kltl_problems);
     ...
     above. */
  {
    ...
    SET_FOREACH(intersect, iter) {
      ...
      boolean self_has_init, self_has_next, self_has_invar, self_has_urgent;
      boolean other_has_init, other_has_next, other_has_invar, other_has_urgent;
      ...
      self_has_urgent = (Nil != FlatHierarchy_lookup_assign(self, var));
      ...
      other_has_urgent = (Nil != FlatHierarchy_lookup_assign(other, var));
      ...
      /* No duplicate urgent assignments */
      nusmv_assert(!(self_has_urgent && other_has_urgent));
   ...
   * [VS] note that no simplification occurs. MR said that should be ok */
    SET_FOREACH(vars, vars_iter) {
      ...
      con = FlatHierarchy_lookup_constraints(other, var);
      if (Nil != con) {
        FlatHierarchy_add_constraints(self, var, con);
      }
      ...
      con = FlatHierarchy_lookup_constraints(other, ivar);
      if (Nil != con) {
        FlatHierarchy_add_constraints(self, ivar, con);
      }
      ...
      con = FlatHierarchy_lookup_constraints(other, nvar);
      if (Nil != con) {
        FlatHierarchy_add_constraints(self, nvar, con);
      }
                                    ...
                                    node_ptr expr, int type)
{
  ...
  if (node_get_type(expr) == separator_symbol) {
    ...
    if (Set_IsEmpty(deps) &&
      ...
      FlatHierarchy_add_constant_constraints(self, expr, type);
    }
    ...
    SET_FOREACH(deps, iter) {
      ...
      FlatHierarchy_add_constraints_type(self, index, expr, type);
    } /* for loop*/
