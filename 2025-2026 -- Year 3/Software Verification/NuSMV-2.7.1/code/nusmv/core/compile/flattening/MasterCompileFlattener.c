                                 ...
                                 MasterCompileFlattener_def_mode def_mode)
{
...

  {
    ...
    StreamMgr_print_error(streams,  "Error: no compatible flattener found for expression:\n");
    StreamMgr_nprint_error(streams, wffprint, "%N", sexp);
    StreamMgr_print_error(streams,  "\n");
    StreamMgr_nprint_error(streams, sexpprint, "%N", sexp);
                                  ...
                                  MasterCompileFlattener_def_mode def_mode)
{
  ...
  if (SymbTable_is_symbol_var(symb_table, atom)) result = atom;
    ...
    if (Flattener_Expand_Def_Mode == def_mode ||
        INTERNAL_ARRAY_STRUCT == node_get_type(exp))
      result = exp;
    else result = atom;
  }
