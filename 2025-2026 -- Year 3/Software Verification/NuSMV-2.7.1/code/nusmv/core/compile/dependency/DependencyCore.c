...
DependencyCore_ptr DependencyCore_create(const NuSMVEnv_ptr env,
                                         const char* name)
{
  const DependencyCore_ptr self = ALLOC(DependencyCore, 1);
...
Set_t dependency_core_get_dependencies(DependencyBase_ptr self,
                                       ...
                                       hash_ptr dependencies_hash)
{
  ...
  case ATOM:
    {
      const ResolveSymbol_ptr rs =
        SymbTable_resolve_symbol(symb_table, formula, context);
      const node_ptr name = ResolveSymbol_get_resolved_name(rs);
  ...
  case ATTIME: /* the variable without time information are returned */
    {
      const int time2 = ExprMgr_attime_get_time(exprs, formula);
      result = _THROW(symb_table, car(formula), context, filter,
                      preserve_time, time2,
                      dependencies_hash);
      break;
  ...
  case BOUNDED_UNTIL:
  case BOUNDED_SINCE:
    /* get internal UNTIL/SINCE operator, ignore bounds */
    result = _THROW(symb_table, car(formula), context, filter,
                    preserve_time, time, dependencies_hash);
    break;
  ...
  case MINU:
    {
      const Set_t right = _THROW(symb_table, cdr(formula), context, filter,
                                 preserve_time, time, dependencies_hash);
  ...
  case IFTHENELSE:
    {
      const Set_t then_arg  =
        _THROW(symb_table, cdr(car(formula)), context, filter,
               preserve_time, time, dependencies_hash);

      const Set_t else_arg  =
        _THROW(symb_table, cdr(formula), context, filter,
               preserve_time, time, dependencies_hash);
  ...
  case BIT_SELECTION:
    {
      const Set_t high_bit =
        _THROW(symb_table, car(cdr(formula)), context, filter,
               preserve_time, time, dependencies_hash);
      const Set_t low_bit =
        _THROW(symb_table, cdr(cdr(formula)), context, filter,
               preserve_time, time, dependencies_hash);
  ...
  case COUNT:
    {
      ...
      while (Nil != list) {
        const Set_t tmp = _THROW(symb_table, car(list), context, filter,
                                 preserve_time, time, dependencies_hash);
  ...
  case CAST_TO_UNSIGNED_WORD:
    {
      const Set_t location =
        _THROW(symb_table, car(formula), context, filter,
               preserve_time, time, dependencies_hash);
  ...
  case ARRAY:
    {
     const ResolveSymbol_ptr rs =
      ...
      if (ResolveSymbol_is_defined(rs)) {
        ...
        const Set_t tmp = _THROW(symb_table, cdr(formula), context, filter,
                                 preserve_time, time, dependencies_hash);
