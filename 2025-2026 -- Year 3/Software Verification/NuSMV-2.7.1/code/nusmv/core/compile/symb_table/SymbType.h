...
/*!
  ...
  The types can be:
    ...
    f) clock, that represent a discrete/continuous variables that represents
       time.
    g) word of width N (N is an integer positive number), that represents
       bit vectors of N bits.
    h) WordArray of address word of width M and value word of width N
    ...
    i) Array represents array of fixed lower and upper bounds
       and elements of a particular type.
    j) Set types created by "union" expressions and used in "in", "case"
        and ":=" expressions:
       ...
       have these types.
    k) No-type is an artificial type to represent expressions which usually
...
typedef enum SymbTypeTag_TAG {
  ...
  SYMB_TYPE_SET_REAL,  /* a set of real values */
...
/*!
  \methodof SymbType
  \brief Returns true if the type is a clock type, or else returns
   false
...
/*!
  \brief Returns the minimal type to which both given types
...
/*!
  \methodof SymbType
  \brief Returns the type represented by a node read from parser

  The SymbType returned is the one corresponding the the a type
  specified at parsing time. This function assumes the type are fully
  resolved (e.g. that the size of an array, or word array are fully
  resolved, no reference to DEFINE. The result type will be memory
  shared.

  Notice that, the constants in the given types (e.g. for enumeratives
  are not stored in any layer. If this is needed, then one has to use
  a different approach, e.g. leveraging on
