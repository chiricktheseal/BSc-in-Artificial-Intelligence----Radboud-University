...
typedef struct SymbCache_TAG
{
  ...
  int enum_vars_num;
  int continuous_vars_num;
  int infinite_precision_vars_num;
...
int SymbCache_get_enum_vars_num(const SymbCache_ptr self)
{
  SYMB_CACHE_CHECK_INSTANCE(self);
  return self->enum_vars_num;
}

int SymbCache_get_continuous_vars_num(const SymbCache_ptr self)
{
  SYMB_CACHE_CHECK_INSTANCE(self);
  return self->continuous_vars_num;
}

int SymbCache_get_infinite_precision_vars_num(const SymbCache_ptr self)
{
  SYMB_CACHE_CHECK_INSTANCE(self);
  return self->infinite_precision_vars_num;
}
...
boolean
SymbCache_is_symbol_array_var_element(const SymbCache_ptr self,
                                      const node_ptr name)
{
  ...
  */
  if (node_get_type(name) == ARRAY &&
    ...
    if (SymbCache_is_symbol_variable_array(self, car(name))) {
      SymbType_ptr type = SymbCache_get_variable_array_type(self, car(name));
    ...
    else return SymbCache_is_symbol_array_var_element(self, car(name));
  }
                            ...
                            NuSMVEnv_ptr env)
{
  ...
  self->enum_vars_num = 0;
  self->continuous_vars_num = 0;
  self->infinite_precision_vars_num = 0;
}
                      ...
                      SymbolInfo* symbol_info)
{
  ...
  case STT_VARIABLE_ARRAY: {
    SymbType_ptr type = SI_VAR_TYPE(symbol_info);
    if (SymbType_is_enum(type) && !SymbType_is_boolean(type)) {
      ...
      if (SymbType_is_continuous(type)) {
        ++self->continuous_vars_num;
      }
      ++self->infinite_precision_vars_num;
    }
    break;
  }
  default:
                         ...
                         const boolean shrink_if_needed)
{
  ...
  case STT_STATE_VAR:
  case STT_VARIABLE_ARRAY: {
    SymbType_ptr type = SI_VAR_TYPE(si);
    if (SymbType_is_enum(type) && !SymbType_is_boolean(type)) {
      --self->enum_vars_num;
    }
    else if (SymbType_is_infinite_precision(type)) {
