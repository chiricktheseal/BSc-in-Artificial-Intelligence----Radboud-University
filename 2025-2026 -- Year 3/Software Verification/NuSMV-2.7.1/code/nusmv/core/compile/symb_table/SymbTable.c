...
#include "nusmv/core/compile/symb_table/SymbTable_private.h"
...
static void
symb_table_unmark_non_continuous_var_on_delete(const SymbTable_ptr table,
                                               const node_ptr sym,
...
static void symb_table_copy_partial(const SymbTable_ptr self, SymbTable_ptr other);
...
SymbLayer_ptr SymbTable_create_layer_extended
                                    (SymbTable_ptr self,
                                     const char* layer_name,
                                     const LayerInsertPolicy ins_policy,
                                     boolean always_create)
{
  char tmp_name[50];

  if (always_create && layer_name != (char*) NULL) {
    int c = snprintf(tmp_name, sizeof(tmp_name) / sizeof(tmp_name[0]),
                     "%s%d", layer_name, self->temp_layer_name_suffix);
  ...
  return SymbTable_create_layer(self, layer_name, ins_policy);
}
                                             ...
                                             const array_t* layer_names)
{
  ...
  SymbTable_iter_set_filter(self, &iter,
                            SymbTableIterFilterFun_take_sf_symbols, NULL);
                                            ...
                                            const array_t* layer_names)
{
  ...
  SymbTable_gen_iter(self, &iter, STT_ALL);
  SymbTable_iter_set_filter(self, &iter,
                            SymbTableIterFilterFun_take_i_symbols, NULL);
                                  ...
                                  const array_t* layer_names)
{
  ...
  SymbTable_gen_iter(self, &iter, STT_ALL);
  SymbTable_iter_set_filter(self, &iter,
                            SymbTableIterFilterFun_take_sf_i_symbols, NULL);
                          ...
                          const node_ptr name)
{
  ...
  else if (SymbCache_is_symbol_array_define(self->cache, name)) {
    node_ptr expansion = SymbTable_get_array_define_flatten_body(self, name);
...
SymbTable_ptr SymbTable_copy(const SymbTable_ptr self, Set_t blacklist)
{
  ...
  new_st = ALLOC(SymbTable, 1);
  SYMB_TABLE_CHECK_INSTANCE(new_st);

  symb_table_copy_partial(self, new_st);
    ...
    SYMB_LAYER_FOREACH(old_layer, iter, STT_ALL) {
      ...
      if (!Set_IsMember(blacklist, symbol)) {
        ...
        if (SymbTable_is_symbol_var(self, symbol)) {
          ...
          if (SymbType_is_enum(type)) {
            ...
            while (value_list != Nil) {
              ...
              if (SymbLayer_can_declare_constant(new_layer, name)) {
                ...
                /* set new symbol as non-continuous if old one is non-continuous */
                if (SymbTable_is_var_non_continuous(self, name)) {
                  SymbTable_mark_var_as_non_continuous(new_st, name);
                }
              }
          ...
          /* set new symbol as non-continuous if old one is non-continuous */
          if (SymbTable_is_var_non_continuous(self, symbol)) {
            SymbTable_mark_var_as_non_continuous(new_st, symbol);
          }
        }
        /* Not variables */
        else {
          if (SymbTable_is_symbol_constant(self, symbol)) {
            ...
            /* set new symbol as non-continuous if old one is non-continuous */
            /* arrays can be flagged as non-continuous */
            if (SymbTable_is_var_non_continuous(self, symbol)) {
              SymbTable_mark_var_as_non_continuous(new_st, symbol);
            }
          }
...
#if !defined(_MSC_VER)
#warning [AMa] FlatHierarchy workaround. See mantis issue #1078
#endif
...
#if USE_CACHED_NUMS
  if (SymbCache_get_infinite_precision_vars_num(self->cache) >0) return true;
  return false;
#else
  {
    SymbTableIter iter;
...
boolean
SymbTable_contains_continuous_variables(const SymbTable_ptr self)
{
#if USE_CACHED_NUMS
  if (SymbCache_get_continuous_vars_num(self->cache) > 0) return true;
  return false;
#else
  SymbTableIter iter;
  ...
  SYMB_TABLE_FOREACH(self, iter, STT_VAR) {
    ...
    if (SymbType_is_continuous(type)) {
      return true;
    }
  }
  return false;
...
boolean
SymbTable_contains_clock_variables(const SymbTable_ptr self)
{
  SymbTableIter iter;
...
{
#if USE_CACHED_NUMS
  if (SymbCache_get_enum_vars_num(self->cache) > 0) return true;
  return false;
#else
  SymbTableIter iter;
                                      ...
                                      const node_ptr name)
{
  ...
  */
  ...
  else if (node_get_type(name) == ARRAY &&
           node_get_type(cdr(name)) == NUMBER_UNSIGNED_WORD) {
    if (SymbTable_is_symbol_variable_array(self, car(name))) {
      SymbType_ptr type = SymbTable_get_variable_array_type(self, car(name));
      WordNumber_ptr val = WORD_NUMBER(car(cdr(name)));
      int width = SymbType_get_wordarray_awidth(type);
      int val_width = WordNumber_get_width(val);
...
boolean SymbTable_is_included(const SymbTable_ptr self,
                              const SymbTable_ptr other)
{
  SymbTableIter iter;
#ifdef SYMB_TABLE_IS_INCLUDED_DEBUG
  NuSMVEnv_ptr const env = EnvObject_get_environment(ENV_OBJECT(self));
  ...
  SYMB_TABLE_FOREACH(self, iter, STT_ALL) {
    node_ptr symbol = SymbTable_iter_get_symbol(self, &iter);

    if (SymbTable_is_symbol_declared(other, symbol)) {
      SymbCategory self_category;
      SymbCategory other_category;
      SymbType_ptr self_type;
      SymbType_ptr other_type;

      self_category = SymbTable_get_symbol_category(self, symbol);
      other_category = SymbTable_get_symbol_category(other, symbol);

      if (self_category != other_category) {
#ifdef SYMB_TABLE_IS_INCLUDED_DEBUG
        StreamMgr_nprint_error(streams, wffprint, "%s: symbol %N: different category\n",
                               __func__, symbol);
#endif
        return false;
      }

      self_type = SymbTable_get_symbol_type(self, symbol);
      other_type = SymbTable_get_symbol_type(other, symbol);

      if (! SymbType_equals(self_type, other_type)) {
#ifdef SYMB_TABLE_IS_INCLUDED_DEBUG
        StreamMgr_nprint_error(streams, wffprint, "%s: symbol %N: different type\n",
                               __func__, symbol);
#endif
        return false;
      }
...
#ifdef SYMB_TABLE_IS_INCLUDED_DEBUG
      StreamMgr_nprint_error(streams, wffprint, "%s: symbol %N: symbol missing\n",
                             __func__, symbol);
#endif
      return false;
    }
...
void SymbTable_mark_var_as_non_continuous(const SymbTable_ptr self,
                                          const node_ptr var)
{
  nusmv_assert(node_get_type(var) == DOT);
  /* add trigger to symb table: when a symbol is removed,
     remove it from the non_continuous;
     notice that adding multiple times the same trigger has no effect */
  SymbTable_add_trigger(self, symb_table_unmark_non_continuous_var_on_delete,
                        ST_TRIGGER_SYMBOL_REMOVE, NULL, false);
  self->non_continuous_vars = Set_AddMember(self->non_continuous_vars, var);
}

void SymbTable_unmark_non_continuous_var(const SymbTable_ptr self,
                                         const node_ptr var)
{
  self->non_continuous_vars = Set_RemoveMember(self->non_continuous_vars, var);
}

void SymbTable_unmark_all_non_continuous_vars(const SymbTable_ptr self)
{
  Set_ReleaseSet(self->non_continuous_vars);
  self->non_continuous_vars = Set_MakeEmpty();
}


boolean SymbTable_is_var_non_continuous(const SymbTable_ptr self,
                                        const node_ptr var)
{
  return Set_IsMember(self->non_continuous_vars, var);
}
...
/*!
  \brief perform a partial copy of self: in particular layers are not copied
...
static void symb_table_copy_partial(const SymbTable_ptr self, SymbTable_ptr other)
{
  ...
  other->streams = self->streams;
  other->nodes = self->nodes;
  other->printer = self->printer;
  other->strings = self->strings;

  other->cache = SymbCache_create(
      other, EnvObject_get_environment(ENV_OBJECT(other)));

  other->categories = new_assoc();  /* not copied, used for memoization */
  other->temp_layer_name_suffix = self->temp_layer_name_suffix;
  other->layers = NodeList_create();
  other->class_layers = new_assoc();
  nusmv_assert(NULL != other->class_layers);
  other->class_names = NodeList_create();
  other->default_class_name = (
      (NULL != self->default_class_name) ?
      util_strsav(self->default_class_name) :
      NULL);

  other->name2layer = new_assoc();
  nusmv_assert(NULL != other->name2layer);

  other->det_counter = self->det_counter;

  other->tc = TypeChecker_copy(self->tc, other);

  other->resolver = ResolveSymbol_create();

  other->handled_structures = new_assoc();
  nusmv_assert(NULL != other->handled_structures);

  other->non_continuous_vars = SET_T(NULL);
}
                                                    ...
                                                    const Expr_ptr expr)
{
  ...
  if (NEXT == node_get_type(expr)) {
    ...
    else if (SymbTable_is_symbol_array_define(st, expr)) {
      node_ptr expanded = SymbTable_get_array_define_flatten_body(st, expr);
      cat = symb_table_detect_expr_category(st, expanded);
    }
    else if (SymbTable_is_symbol_define(st, expr)) {
      node_ptr expanded = SymbTable_get_define_flatten_body(st, expr);
      cat = symb_table_detect_expr_category(st, expanded);
    }
  }
  else if (SymbTable_is_symbol_declared(st, expr) &&
...
/*!
  \brief trigger used to unmark non continuous vars when they are deleted.
...
symb_table_unmark_non_continuous_var_on_delete(const SymbTable_ptr table,
                                               ...
                                               void* arg)
{
  nusmv_assert(arg == NULL);
  nusmv_assert(action == ST_TRIGGER_SYMBOL_REMOVE);
  SymbTable_unmark_non_continuous_var(table, sym);
}
