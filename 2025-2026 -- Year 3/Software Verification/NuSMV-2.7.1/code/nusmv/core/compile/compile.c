...
static int _compile_get_bits(const SymbTable_ptr st, const SymbType_ptr type);
                                ...
                                OStream_ptr output_file)
{
  ...
  /* Do some post-processings over the COI hierarchy */
  {
    ...
    /* 1. -> INIT, TRANS, INVAR, INPUT, JUSTICE, COMPASSION. */
    {
      ...
      if (FlatHierarchy_get_urgent(fsm_fh) != Nil) {
        error_unreachable_code_msg("COI of models with URGENT constraints " \
                                   "is not supported");
      }
...
int Compile_get_bits(const SymbTable_ptr st, const NodeList_ptr lst)
{
  ...
  NODE_LIST_FOREACH(lst, iter) {
    ...
    if (SymbTable_is_symbol_variable_array(st, var)) {
      type = SymbTable_get_variable_array_type(st, var);
    } else {
      type = SymbTable_get_var_type(st, var);
    }

    res += _compile_get_bits(st, type);
  }
...
void Compile_write_global_coi_fsm(NuSMVEnv_ptr env,
                                  ...
                                  OStream_ptr output_file)
{
  struct ProblemDbIter prop_iter;
  ...
  PROBLEM_DB_ACCESSOR_FOREACH(prop_dba, prop_iter) {
    Prop_ptr prop = PropDbAccessor_get_prop_at_iter(prop_dba, &prop_iter);
                                 ...
                                 const char* file_name)
{
  ...
  NodeList_ptr order =
    PropDbAccessor_get_coi_grouped_properties(
        PROP_DB_ACCESSOR(NuSMVEnv_get_value(env, ENV_PROP_DBA)), hierarchy);
  ListIter_ptr iter;
  int prop_no = 0;
                        ...
                        OStream_ptr file, node_ptr ntype, int threshold)
{
  ...
  case INTEGER:
    ...
    break;
  case CLOCK:
    OStream_printf(file, "clock\n");
    break;
  case WORDARRAY_TYPE:
    OStream_printf(file, "array word[%d] of ", PTR_TO_INT(car(ntype)));
    Compile_print_type(env, file, cdr(ntype), threshold);
    break;
  case INTARRAY_TYPE:
    OStream_printf(file, "array integer of ");
    Compile_print_type(env, file, car(ntype), threshold);
    break;
  default:
                              ...
                              SymbType_ptr type)
{
  ...
  case SYMB_TYPE_CLOCK:
    ntype = find_node(nodemgr, CLOCK, Nil, Nil);
    break;
  ...
  case SYMB_TYPE_WORDARRAY:
    {
      node_ptr subtype =
        Compile_get_var_type(env, SymbType_get_wordarray_subtype(type));
      j = SymbType_get_wordarray_awidth(type);
      ntype = find_node(nodemgr, WORDARRAY_TYPE, PTR_FROM_INT(node_ptr, j),
                        subtype);
    }
    break;
  case SYMB_TYPE_INTARRAY:
    {
      node_ptr subtype = Compile_get_var_type(env, SymbType_get_intarray_subtype(type));
      ntype = find_node(nodemgr, INTARRAY_TYPE, subtype, Nil);
    }
    break;
  default:
    ErrorMgr_rpterr(errmgr, "Unsupported type found.");
  ...
  if (!total_only && !defs_only) {
    ...
    { /* prints the defines */
      ...
      SYMB_TABLE_FOREACH(st, iter, STT_DEFINE) {
        ...
        SymbType_print(type, wffprint, OStream_get_stream(ostream));
        OStream_printf(ostream,  "\n");
      } /* for */
    }
    OStream_reset_indent_size(ostream);
...
void Compile_print_predicates(const NuSMVEnv_ptr env)
{
  ...
  MasterCompilePredicateNormaliser_ptr normalizer = MasterCompilePredicateNormaliser_create(st);
  node_ptr tmp;
  array_t* layers_name;
  int i;
  const char* a_layer_name;

  if (FlatHierarchy_get_urgent(hierarchy) != Nil) {
    error_unreachable_code_msg("Print predicates of models with URGENT " \
  ...
  */
  ...
  tmp = MasterCompilePredicateNormaliser_normalise_expr(normalizer, tmp, true);
  MasterCompilePredicateNormaliser_get_predicates_only(normalizer, &predicates, tmp);
  ...
  */
  ...
  arrayForEachItem(const char*, layers_name, i, a_layer_name) {
    ...
    SYMB_LAYER_FOREACH(layer, iter, STT_VAR) {
      ...
      if (invar_expr != Nil) {
        tmp = MasterCompilePredicateNormaliser_normalise_expr(
            normalizer, find_node(nodemgr, EQDEF, name, invar_expr), true);
        MasterCompilePredicateNormaliser_get_predicates_only(normalizer, &predicates, tmp);
      }
      if (init_expr != Nil) {
        tmp = MasterCompilePredicateNormaliser_normalise_expr(
            normalizer, find_node(nodemgr, EQDEF, init_name, init_expr), true);
        MasterCompilePredicateNormaliser_get_predicates_only(normalizer, &predicates, tmp);
      }
      if (next_expr != Nil) {
        tmp = MasterCompilePredicateNormaliser_normalise_expr(
            normalizer, find_node(nodemgr, EQDEF, next_name, next_expr), true);
        MasterCompilePredicateNormaliser_get_predicates_only(normalizer, &predicates, tmp);
      }
    }
  }
  ...
  MasterCompilePredicateNormaliser_destroy(normalizer);
}
...
static int _compile_get_bits(const SymbTable_ptr st, const SymbType_ptr type)
{
  ...
  if (SymbType_is_boolean(type)) {
    ...
    SymbType_ptr subtype = SymbType_get_wordarray_subtype(type);

    if (SymbType_is_boolean(subtype) ||
        SymbType_is_enum(subtype) ||
        SymbType_is_word(subtype)) {
      res += SymbType_get_wordarray_awidth(type) *
        _compile_get_bits(st, subtype);
    }
    else {
      if (opt_verbose_level_gt(opts, 0)) {
        StreamMgr_print_error(streams,
        ...
        StreamMgr_print_error(streams, " not possible to compute bits.\n");
      }
    }
  }
  else {
