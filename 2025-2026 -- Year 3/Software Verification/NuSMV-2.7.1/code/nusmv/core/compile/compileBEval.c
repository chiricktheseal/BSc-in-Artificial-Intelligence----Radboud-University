                            ...
                            Expr_ptr expr)
{
...
#if ENABLE_PN_BEFORE_CONVERSION
  {
    ...
    if (opt_boolconv_uses_prednorm(opts)) {
      MasterCompilePredicateNormaliser_ptr pn;
      pn = MasterCompilePredicateNormaliser_create(symb_table);
      expr = MasterCompilePredicateNormaliser_normalise_expr(pn, expr, true);
      MasterCompilePredicateNormaliser_destroy(pn);
    }
  }
#endif
                                      ...
                                      hash_ptr cache)
{
  ...
  hash_ptr non_booleanizable = (hash_ptr)NuSMVEnv_get_value(env, ENV_BOOLEANIZEABLE_TYPE);
...
static Expr_ptr apply_time_on_expr(BddEnc_ptr bdd_enc, Expr_ptr expr, int time)
{
  ...
  case TIME:
    ErrorMgr_rpterr(errmgr, "time can not be casted to boolean");
    break;
  ...
  case NEXT:
    {
      res = apply_time_on_expr(bdd_enc, car(expr), time+1);
      break;
    }
  case NOT:
    {
                                 ...
                                 hash_ptr expr2bexpr)
{
  ...
  case CAST_TOINT: {
    ...
    body = Compile_FlattenSexp(symb_table, car(expr), context);
    type = TypeChecker_get_expression_type(
      SymbTable_get_type_checker(symb_table), body, Nil);

    if (SymbType_is_word(type)) {
      node_ptr tmp = compile_flatten_rewrite_word_toint_cast(env, body, type);
  ...
  case INVARSPEC:
  case COMPUTE:
    /* This is more robust since if the whole expression is
       booleanized, or the expression has already been flattened, the
       CONTEXT is no longer present in the expression */
    if (CONTEXT == node_get_type(car(expr))) {
      res =
      ...
      res =
        expr2bexpr_recur(enc, det_layer, car(expr), Nil, in_next,
                         expr2bexpr);
    }
    break;
                                  ...
                                  boolean in_next)
{
  ...
  node_ptr res;
  int temp = nusmv_yylineno;
  add_ptr bool_add = (add_ptr)NULL;

  {
    /* Ensure the layers committed so far in the BoolEnc have been
     committed to the BddEnc.  Indeed, if the conversion from scalar
     to bool reaches this point, it must rely on BDDs to compute the
     boolean expression, and it must be ensure the BDDs have been
     created. We cannot consider all the layers since the det_layer is
     not yet commited, and if we add it, then we try to add it twice,
     and possibly not considering all the variables declared within
     it. */
    BoolEnc_ptr benc = BOOL_ENC(BoolEncClient_get_bool_enc(BOOL_ENC_CLIENT(enc)));
    NodeList_ptr layers = BaseEnc_get_committed_layers(BASE_ENC(benc));
    BaseEnc_commit_layers_list(BASE_ENC(enc), layers);
  }

  bool_add = BddEnc_expr_to_add(
      enc, (in_next) ?
                             ...
                             const CompileIsBooleanizableAuxParams* params)
{
  ...
  case ARRAY:
    {
      ...
      if (ResolveSymbol_is_var(rs)) {
        ...
        res = !SymbType_is_real(ty);
               ...
               SymbTable_is_symbol_variable_array(st, resolved)) {
        SymbType_ptr ty = SymbTable_get_variable_array_type(st, resolved);
        SymbType_ptr sty = SymbType_get_array_subtype(ty);

        /* Real, integers, strings, unbounded arrays .. */
        res = ! (SymbType_is_infinite_precision(sty) ||
                 SymbType_is_intarray(sty) ||
                 SymbType_is_wordarray(sty));

        if (res && SymbType_is_word(sty)) {
          res = !params->word_unbooleanizable;
        }
      }
        ...
        res = compile_is_booleanizable_aux(tmp, Nil, params);
      }
      else {
          StreamMgr_print_error(params->streams, "Unexpected symbol in Compile_is_expr_booleanizable: ");
          StreamMgr_nprint_error(params->streams, params->wffprint, "'%N'\n", expr);
  ...
  default:
    {
      if (((hash_ptr)NULL != params->non_booleanizable) &&
          find_assoc(params->non_booleanizable, NODE_FROM_INT(node_get_type(expr)))) {
        res = false;
      }
      else {
        /* Lazy evaluation */
                                     ...
                                     hash_ptr non_booleanizable)
{
  ...
  self->wffprint = MASTER_PRINTER(NuSMVEnv_get_value(env, ENV_WFF_PRINTER));
  self->word_unbooleanizable = word_unbooleanizable;
  self->cache = cache;
  self->enable_debugging = false;
  self->non_booleanizable = non_booleanizable;

  if (OptsHandler_is_option_registered(opts,
                                       "debug_compile_is_booleanizable")) {
