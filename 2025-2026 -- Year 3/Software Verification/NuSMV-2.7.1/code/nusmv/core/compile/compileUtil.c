...
node_ptr sym_intern(const NuSMVEnv_ptr env, const char* s)
{
  ...
  const string_ptr escaped = UStringMgr_escape_string_for_atom(strings, s);

  const node_ptr retval =
    find_node(nodemgr, ATOM, (node_ptr)escaped, Nil);
                            ...
                            node_ptr name1, node_ptr name2)
{
  ...
  case NUMBER:
      nusmv_assert(Nil == cdr(name1));
      nusmv_assert(Nil == cdr(name2));
      return PTR_TO_INT(car(name1)) == PTR_TO_INT(car(name2));
...
/*!
  \brief Private macro for checking whether the temporal operator
    OP_GLOBAL is unbounded.
 */
#define IS_UNBOUNDED_OP_GLOBAL(prop)            \
...
/*!
  \brief Private macro for checking whether the temporal operator
    OP_HISTORICAL is unbounded.
 */
#define IS_UNBOUNDED_OP_HISTORICAL(prop)            \
...
/*!
  \brief Private macro for checking whether the temporal operator
    OP_FUTURE is unbounded.
 */
#define IS_UNBOUNDED_OP_FUTURE(prop)            \
...
/*!
  \brief Private macro for checking whether the temporal operator
    OP_ONCE is unbounded.
 */
#define IS_UNBOUNDED_OP_ONCE(prop)            \
...
#define IS_AND_DISTRIB_OP(prop)                \
  ((IS_UNBOUNDED_OP_GLOBAL(prop)) ||           \
  (AG == node_get_type(prop))  ||              \
  (IS_UNBOUNDED_OP_HISTORICAL(prop)))
...
#define IS_OR_DISTRIB_OP(prop)                 \
  ((IS_UNBOUNDED_OP_FUTURE(prop)) ||           \
  (EF == node_get_type(prop))  ||              \
  (IS_UNBOUNDED_OP_ONCE(prop)))
...
static node_ptr compile_remove_ltl_bop_recurse(NodeMgr_ptr nodemgr,
  ...
  case OP_HISTORICAL:
    ...
    if (Nil != cdr(prop)) {
      const node_ptr condition = compile_remove_ltl_bop_recurse(nodemgr, car(prop));
      const node_ptr nt1 = car(cdr(prop));
      const node_ptr nt2 = cdr(cdr(prop));
      node_ptr result = condition;
      int OP = -1, TOP = -1;
      int t1, t2, j;

      nusmv_assert(TWODOTS == node_get_type(cdr(prop)));
      /* This function handles only the discrete case. */
      nusmv_assert(NUMBER == node_get_type(nt1));
      nusmv_assert(NUMBER == node_get_type(nt2));

      t1 = node_get_int(nt1);

      nusmv_assert(t1 >= 0);

      if (OP_HISTORICAL == node_get_type(prop)) {
        TOP = OP_NOTPRECNOT;
        OP = AND;
      }
      else {
      ...
      else { /* not an integer: leave it as it is */
        /* go forward/backward of t1 steps and evaluate the expression.
           Here we reconstruct the original expression in without the bounded
           LTL operators */
        result = find_node(nodemgr, node_get_type(prop), result, Nil);
      }
  ...
  case BOUNDED_SINCE:
    { /*
        Get the 2 bounds: t1 and t2.
        Let lhs and rhs be the 2 subformulas that are operators of the inner
        UNTIL/SINCE expression: `lhs U/S [t1, t2] rhs`.
        rhs must hold at some point between t1 and t2,
        lhs must hold from now until/since that point.
      */
      const int top =
        (node_get_type(prop) == BOUNDED_UNTIL)? OP_NEXT : OP_PREC;
      const node_ptr nt1 = car(cdr(prop));
      const node_ptr nt2 = cdr(cdr(prop));
      /* lhs U/S rhs : recursively remove bounded operators from lhs and rhs. */
      const node_ptr lhs = compile_remove_ltl_bop_recurse(nodemgr, car(car(prop)));
      const node_ptr rhs = compile_remove_ltl_bop_recurse(nodemgr, cdr(car(prop)));
      node_ptr result = rhs;
      int t1, t2, j;

      nusmv_assert(cdr(prop) != Nil);
      nusmv_assert(node_get_type(nt2) == NUMBER);
      nusmv_assert(node_get_type(prop) != BOUNDED_UNTIL ||
                   node_get_type(car(prop)) == UNTIL);
      nusmv_assert(node_get_type(prop) != BOUNDED_SINCE ||
                   node_get_type(car(prop)) == SINCE);
      nusmv_assert(node_get_type(cdr(prop)) == TWODOTS);
      nusmv_assert(nt1 != Nil);
      nusmv_assert(node_get_type(nt1) == NUMBER);
      nusmv_assert(nt2 != Nil);
      ...
      for (j = t2 - t1; j > 0; j--) {
        /* Either rhs holds now, or lhs holds/held the next/previous step and
           also results holds/held there.
           Notice that results has been initialized to rhs.
           rhs | (lhs & X/Y result) */
        result = find_node(nodemgr, OR, rhs,
                           find_node(nodemgr, AND, lhs,
                                     find_node(nodemgr, top, result, Nil)));
      }
      /* lhs & X/Y result */
      for (j = 0; j < t1; ++j) {
        result = find_node(nodemgr, AND, lhs,
                           find_node(nodemgr, top, result, Nil));
      }
      return result;
    }
