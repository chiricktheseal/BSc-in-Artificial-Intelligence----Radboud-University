...
int BaseEnc_commit_layers_list(BaseEnc_ptr self,
                               const NodeList_ptr layers)
{
  int res = 0;
  const char* layer_name;

  ListIter_ptr it;
  NODE_LIST_FOREACH(layers, it) {
    layer_name = SymbLayer_get_name((SymbLayer_ptr)NodeList_get_elem_at(layers, it));
    if (!BaseEnc_layer_occurs(self, layer_name)) {
      BaseEnc_commit_layer(self, layer_name);
      ++res;
    }
