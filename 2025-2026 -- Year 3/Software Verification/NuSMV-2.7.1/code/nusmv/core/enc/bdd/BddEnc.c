...
NodeList_ptr BddEnc_get_vars_in_cube_NodeList
                                    (const BddEnc_ptr self,
                                     bdd_ptr cube,
                                     NodeList_ptr list_of_sym,
                                     boolean include_next)
{
  add_ptr add_cube;
  NuSMVEnv_ptr env;
  ...
  env = EnvObject_get_environment(ENV_OBJECT(self));
  dd = BddEnc_get_dd_manager(self);
  add_cube = bdd_to_add(dd, cube);
  nodemgr = NODE_MGR(NuSMVEnv_get_value(env, ENV_NODE_MGR));

  NODE_LIST_FOREACH(list_of_sym, iter) {
    node_ptr sym = NodeList_get_elem_at(list_of_sym, iter);
    if (BddEnc_is_var_in_cube(self, sym, add_cube)) {
      NodeList_append(result, sym);
    }
    if (include_next) {
...
void bdd_enc_remove_layer(BaseEnc_ptr enc_base, const char* layer_name)
{
  ...
  while (layers[i] != SYMB_LAYER(NULL)) {
    ...
    SYMB_LAYER_FOREACH_FILTER(layers[i], iter, STT_VAR,
                              SymbLayerIterFilterFun_take_bool_vars,
                              BASE_ENC(self)->symb_table) {
                                     ...
                                     boolean* layers_interleaved)
{
  ...
  *layers_interleaved = false;
  ...
  NODE_LIST_FOREACH(layers, layer_iter) {
    ...
    if (!BoolEnc_is_bool_layer(SymbLayer_get_name(layer))) {
...
#pragma message ("[MRMR]: Why doing set in here. It should be an IFF.")
  ...
  case CAST_TOINT: {
    /* here we rely on the flattener, which is in charge of getting
       rid of CAST_TOINT */
    //node_ptr wn = Compile_FlattenSexp(BASE_ENC(self)->symb_table, expr, ctx);
    node_ptr body = Compile_FlattenSexp(BASE_ENC(self)->symb_table, car(expr), ctx);
    SymbType_ptr type = TypeChecker_get_expression_type(
      SymbTable_get_type_checker(BASE_ENC(self)->symb_table), body, Nil);

    node_ptr wn = Nil;
    if (SymbType_is_word(type)) {
      wn = compile_flatten_rewrite_word_toint_cast(env, body, type);
    } else {
      wn = Compile_FlattenSexp(BASE_ENC(self)->symb_table, expr, ctx);
    }
  ...
  case WAREAD: {
    node_ptr tmp = Compile_FlattenSexp(BASE_ENC(self)->symb_table, expr, ctx);
    return bdd_enc_eval(self, tmp, Nil);
  }
...
BddEnc_ptr BddEnc_copy(const BddEnc_ptr bdd_enc_old)
{
  ...
  SymbTable_ptr st_old = BaseEnc_get_symb_table(BASE_ENC(bool_enc_old));
  SymbTable_ptr st_new;

  if (opt_verbose_level_gt(opts, 4)) {
    Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
    Logger_log(logger, "[BddEnc_clone] copying a BoolEnc...\n");
  }
  st_new = SymbTable_copy(st_old, Set_MakeEmpty());
  bool_enc_new = BoolEnc_create(st_new);
  BaseEnc_commit_layers(BASE_ENC(bool_enc_new),
                        BaseEnc_get_committed_layer_names(BASE_ENC(bool_enc_old)));
  if (opt_verbose_level_gt(opts, 4)) {
    Logger_ptr logger = LOGGER(NuSMVEnv_get_value(env, ENV_LOGGER));
    Logger_log(logger, "[BddEnc_clone] copied a BoolEnc.\n");
  }

  /* new BddEnc_ptr from old, based on new SymbTable_ptr and new BoolEnc_ptr */
  {
    VarsHandler_ptr dd_vars_hndr = BddEnc_get_dd_vars_handler(bdd_enc_old);
    OrdGroups_ptr ord_groups = OrdGroups_copy(BddEnc_get_ord_groups(bdd_enc_old));

    bdd_enc_new = BddEnc_create(st_new, bool_enc_new, dd_vars_hndr, ord_groups);
    BaseEnc_commit_layers(BASE_ENC(bdd_enc_new),
                          BaseEnc_get_committed_layer_names(BASE_ENC(bdd_enc_old)));
  }
  return bdd_enc_new;
}
