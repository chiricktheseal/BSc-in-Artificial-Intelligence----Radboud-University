                      ...
                      const char* dump_fname_template)
{
  ...
  if (must_solve) {
    ...
    if (result == BMC_TRUE) {
      ...
      if (opt_counter_examples(opts)) {
        /* Temporary trace manager to print the trace */
        TraceMgr_ptr gtm = TraceMgr_create(env);

        TraceMgr_register_trace(gtm, trace);
        StreamMgr_print_output(streams,
                               "-- as demonstrated by the following induction step sequence\n");

        {
          TracePlugin_ptr plugin;
          int index = -1;
          plugin = TRACE_PLUGIN(TraceExplainer_create(true));
          index = TraceMgr_register_plugin(gtm, plugin);

          TraceMgr_execute_plugin(gtm, TRACE_OPT(NULL),
                                  index,
                                  TRACE_MGR_LAST_TRACE);
        }
        TraceMgr_destroy(gtm);
      }
    }
    else {
  ...
  case SAT_SOLVER_SATISFIABLE_PROBLEM:
    ...
    if (opt_counter_examples(opts)) {
      ...
      *trace = Bmc_Utils_generate_cntexample(be_enc,
                                             ...
                                             symbols);
      Trace_set_type(*trace, TRACE_TYPE_INDUCTION_SEQUENCE);
    }
    result = BMC_UNKNOWN;
    break;
