...
int Mc_check_psl_spec(const NuSMVEnv_ptr env, const int prop_no)
{
  ...
  if (prop_no != -1) {
    ...
    if (Prop_check_type(PropDbAccessor_get_prop_at_index(prop_dba,
      ...
      status =
        Mc_check_psl_property(env, PropDbAccessor_get_prop_at_index(prop_dba, prop_no));
    }
  }
  else {
    lsList props;
  ...
  CATCH(errmgr) {
    if (NULL != prop) {
      ...
      if (opt_use_coi_size_sorting(opts)) {
        ...
        PropDbAccessor_ordered_verify_all_type(prop_dba, hierarchy, Prop_Invar);
      }
      else PropDbAccessor_verify_all_type(prop_dba, Prop_Invar);
    }
