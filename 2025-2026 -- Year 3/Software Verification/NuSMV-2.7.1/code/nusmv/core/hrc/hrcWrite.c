                                      ...
                                      boolean append_suffix)
{
  ...
  */
  ...
  if (hrc_write_assign_list(env,
                            ofile,
                            URGENT,
                            HrcNode_get_urgent_assign_exprs_iter(hrcNode))) {
    fprintf(ofile, "\n");
  }

  /* CONSTRAINS: INIT, INVAR, TRANS, URGENT */
  /* JUSTICE/FAIRNESS */
  {
    struct {
      Oiter iter;
      const char* type;
        ...
        {{HrcNode_get_urgent_exprs_iter(hrcNode).node}, "URGENT\n"},
...
/*!
  ...
  CLOCK, UNSIGNED_WORD, SIGNED_WORD, SCALAR, WORD_ARRAY and ARRAY_TYPE.
*/
static void print_variable_type(const NuSMVEnv_ptr env,
                                FILE* out, node_ptr node)
{
