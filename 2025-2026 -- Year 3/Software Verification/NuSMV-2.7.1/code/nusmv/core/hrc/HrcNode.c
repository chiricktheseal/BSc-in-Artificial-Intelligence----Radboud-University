...
void HrcNode_cleanup(HrcNode_ptr self)
{
  ...
  FREELIST_AND_SET_TO_NIL(self->urgent_expr);
  self->urgent_expr = Olist_create();
  ...
  FREELIST_AND_SET_TO_NIL(self->predicates);
  self->predicates = Olist_create();
  FREELIST_AND_SET_TO_NIL(self->mirrors);
  self->mirrors = Olist_create();
  ...
  FREELIST_AND_SET_TO_NIL(self->psynth_probs);
  self->psynth_probs = Olist_create();
  FREELIST_AND_SET_TO_NIL(self->kltl_probs);
  self->kltl_probs = Olist_create();
  FREELIST_AND_SET_TO_NIL(self->property_patterns);
  self->property_patterns = Olist_create();
  ...
  FREE_LIST_AND_SET_TO_NIL(self, self->urgent_assign);
  self->urgent_assign = Olist_create();
...
void HrcNode_add_state_variable(HrcNode_ptr self, node_ptr var)
{
  ...
  nusmv_assert(Nil == var ||
               (CONS == node_get_type(var) &&
                Nil != car(var) && Nil != cdr(var)));
#if HRC_NODE_CHECK_DUPLICATION
...
void HrcNode_remove_frozen_function(HrcNode_ptr self,
                                    node_ptr fun)
{
  nusmv_assert(CONS == node_get_type(fun));

  Olist_remove(self->frozen_functions, fun);
}
...
void HrcNode_replace_urgent_exprs(HrcNode_ptr self, Olist_ptr exprs)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != exprs);
  ...
  self->urgent_expr = exprs;
}

Oiter HrcNode_get_urgent_exprs_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->urgent_expr);
}

void HrcNode_add_urgent_expr(HrcNode_ptr self, node_ptr expr)
{
  HRC_NODE_CHECK_INSTANCE(self);
#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->urgent_expr, expr)) {
    Olist_append(self->urgent_expr, expr);
  }
#else
  Olist_append(self->urgent_expr, expr);
#endif
...
void HrcNode_replace_urgent_assign_exprs(HrcNode_ptr self, Olist_ptr assigns)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != assigns);

  hrc_node_free_list_and_clear_assign_map(self, URGENT);
  /* free the list created by the hrc_node_free_list_and_clear_assign_map */
  nusmv_assert(OLIST(NULL) != self->urgent_assign);
  Olist_destroy(self->urgent_assign);

  hrc_node_insert_assign_hash_list(self, assigns, URGENT);
  self->urgent_assign = assigns;
}

Oiter HrcNode_get_urgent_assign_exprs_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->urgent_assign);
}

void HrcNode_add_urgent_assign_expr(HrcNode_ptr self, node_ptr assign)
{
  HRC_NODE_CHECK_INSTANCE(self);
#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->urgent_assign, assign)) {
    /* checked to be ok for copies as well */
    hrc_node_insert_assign_hash(self, assign, URGENT);
    Olist_append(self->urgent_assign, assign);
  }
#else
  hrc_node_insert_assign_hash(self, assign, URGENT);
  Olist_append(self->urgent_assign, assign);
...
void HrcNode_replace_predicates(HrcNode_ptr self, Olist_ptr predicates)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != predicates);

  FREELIST_AND_SET_TO_NIL(self->predicates);
  self->predicates = predicates;
}

Oiter HrcNode_get_predicate_exprs_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->predicates);
}

void HrcNode_add_predicate(HrcNode_ptr self, node_ptr predicate)
{
  HRC_NODE_CHECK_INSTANCE(self);
#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->predicates, predicate)) {
    Olist_append(self->predicates, predicate);
  }
#else
  Olist_append(self->predicates, predicate);
#endif
...
void HrcNode_replace_mirrors(HrcNode_ptr self, Olist_ptr mirrors)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != mirrors);

  FREELIST_AND_SET_TO_NIL(self->mirrors);
  self->mirrors = mirrors;
}

Oiter HrcNode_get_mirror_exprs_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->mirrors);
}

void HrcNode_add_mirror(HrcNode_ptr self, node_ptr mirror)
{
  HRC_NODE_CHECK_INSTANCE(self);
#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->mirrors, mirror)) {
    Olist_append(self->mirrors, mirror);
  }
#else
  Olist_append(self->mirrors, mirror);
#endif
...
void HrcNode_replace_psynth_problems(
    HrcNode_ptr self, Olist_ptr psynth_probs)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != psynth_probs);

  FREELIST_AND_SET_TO_NIL(self->psynth_probs);
  self->psynth_probs = psynth_probs;
}

Oiter HrcNode_get_psynth_problems_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->psynth_probs);
}

void HrcNode_add_psynth_problem(
    HrcNode_ptr self, node_ptr psynth_prob)
{
  HRC_NODE_CHECK_INSTANCE(self);

#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->psynth_probs, psynth_prob)) {
    Olist_append(self->psynth_probs, psynth_prob);
  }
#else
  Olist_append(self->psynth_probs, psynth_prob);
#endif
...
void HrcNode_replace_kltl_problems(HrcNode_ptr self, Olist_ptr kltl_probs)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != kltl_probs);

  FREELIST_AND_SET_TO_NIL(self->kltl_probs);
  self->kltl_probs = kltl_probs;
}

Oiter HrcNode_get_kltl_problems_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->kltl_probs);
}

void HrcNode_add_kltl_problem(HrcNode_ptr self, node_ptr kltl_prob)
{
  HRC_NODE_CHECK_INSTANCE(self);
#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->kltl_probs, kltl_prob)) {
    Olist_append(self->kltl_probs, kltl_prob);
  }
#else
  Olist_append(self->kltl_probs, kltl_prob);
#endif
...
void HrcNode_replace_property_pattern_expr(HrcNode_ptr self, Olist_ptr prop_patterns)
{
  HRC_NODE_CHECK_INSTANCE(self);
  nusmv_assert(OLIST(NULL) != prop_patterns);

  FREELIST_AND_SET_TO_NIL(self->property_patterns);
  self->property_patterns = prop_patterns;
}

Oiter HrcNode_get_property_pattern_exprs_iter(const HrcNode_ptr self)
{
  HRC_NODE_CHECK_INSTANCE(self);

  return Olist_first(self->property_patterns);
}

void HrcNode_add_property_pattern_expr(HrcNode_ptr self, node_ptr prop_pattern)
{
  HRC_NODE_CHECK_INSTANCE(self);
#if HRC_NODE_AVOID_DUPLICATION
  if (!Olist_contains(self->property_patterns, prop_pattern)) {
    Olist_append(self->property_patterns, prop_pattern);
  }
#else
  Olist_append(self->property_patterns, prop_pattern);
#endif
...
HrcNode_ptr HrcNode_copy(const HrcNode_ptr self)
{
  ...
  copy = HRC_NODE(OBJECT(self)->copy(OBJECT(self)));

  HRC_NODE_CHECK_INSTANCE(copy);
...
node_ptr HrcNode_find_var_all(HrcNode_ptr self, node_ptr var_name)
{
  ...
  int types[] = {VAR, FROZENVAR, IVAR, FUN};
  int i;
...
node_ptr HrcNode_find_var(HrcNode_ptr self, node_ptr var_name, int type)
{
  ...
  case IVAR:
    ...
    break;
  case FUN:
    iter = HrcNode_get_frozen_functions_iter(self);
    break;
  default:
...
void hrc_node_init(HrcNode_ptr self, const NuSMVEnv_ptr env)
{
  ...
  OVERRIDE(Object, copy) = hrc_node_copy;
}
...
void hrc_node_deinit(HrcNode_ptr self)
{
  ...
  FREE_LIST_AND_SET_TO_NIL(self, self->compassion);
  FREELIST_AND_SET_TO_NIL(self->constants);
  FREE_LIST_AND_SET_TO_NIL(self, self->predicates);
  FREE_LIST_AND_SET_TO_NIL(self, self->mirrors);
  FREELIST_AND_SET_TO_NIL(self->ctl_props);
  FREELIST_AND_SET_TO_NIL(self->ltl_props);
  FREELIST_AND_SET_TO_NIL(self->psl_props);
  FREELIST_AND_SET_TO_NIL(self->invar_props);
...
void hrc_node_copy_aux(const HrcNode_ptr self,
                       HrcNode_ptr hrc_copy)
{
  NuSMVEnv_ptr env;
  NodeMgr_ptr nodemgr;
  ...
  env_object_copy_aux(ENV_OBJECT(self), ENV_OBJECT(hrc_copy));
  ...
  hrc_copy->urgent_expr = Olist_copy(self->urgent_expr);
  hrc_copy->justice = Olist_copy(self->justice);
  hrc_copy->constants = Olist_copy(self->constants);
  hrc_copy->predicates = Olist_copy(self->predicates);
  hrc_copy->mirrors = Olist_copy(self->mirrors);
  ...
  hrc_copy->psynth_probs = Olist_copy(self->psynth_probs);
  hrc_copy->kltl_probs = Olist_copy(self->kltl_probs);
  hrc_copy->property_patterns = Olist_copy(self->property_patterns);
  ...
  hrc_copy->urgent_assign = hrc_node_copy_list(nodemgr, self->urgent_assign);
  hrc_copy->compassion = hrc_node_copy_list(nodemgr, self->compassion);
  hrc_copy->predicates = hrc_node_copy_list(nodemgr, self->predicates);
  hrc_copy->mirrors = hrc_node_copy_list(nodemgr, self->mirrors);
  ...
  /* Copy the assign's table */
  {
    ...
    hrc_copy->assigns_table = new_assoc();
  ...
  /* childs are initialized empty */
  hrc_copy->childs = Slist_create();
}

void hrc_node_finalize(Object_ptr object, void* dummy)
{
...
Object_ptr hrc_node_copy(const Object_ptr self)
{
  HrcNode_ptr copy;

  copy = ALLOC(HrcNode, 1);
  HRC_NODE_CHECK_INSTANCE(copy);

  hrc_node_copy_aux(HRC_NODE(self), copy);
