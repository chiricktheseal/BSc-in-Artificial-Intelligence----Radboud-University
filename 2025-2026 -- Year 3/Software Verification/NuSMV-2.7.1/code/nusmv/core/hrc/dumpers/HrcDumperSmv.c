                         ...
                         FILE* fout)
{
  ...
  OVERRIDE(HrcDumper, dump_annotations) = hrc_dumper_smv_dump_annotations;
}
...

  {
    ...
    case HDS_HRC_TOP:
      if (info->stage & HRC_STAGE_BEGIN){
        _HRC_DUMP_HEADER("");
        /* dump model annotations */
        HrcDumper_dump_annotations(HRC_DUMPER(self));
      }
      break;
    ...
    case HDS_SYMBOL:
      if (info->stage & HRC_STAGE_BEGIN) {
        ...
        case SYMBOL_FROZEN_VAR:
          _HRC_DUMP_NODE(info->n1.name);
          _HRC_DUMP_STR(" : ");
          HrcDumper_dump_var_type(self, info->n2.type);
          break;
...
void hrc_dumper_smv_dump_annotations(const HrcDumper_ptr self)
{
}
