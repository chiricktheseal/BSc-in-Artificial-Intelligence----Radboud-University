...
VIRTUAL void HrcDumper_dump_var_type(const HrcDumper_ptr self, node_ptr node)
{
  HRC_DUMPER_CHECK_INSTANCE(self);
  self->dump_var_type(self, node);
}


VIRTUAL void HrcDumper_dump_annotations(const HrcDumper_ptr self)
{
  HRC_DUMPER_CHECK_INSTANCE(self);
  self->dump_annotations(self);
}
...
void hrc_dumper_init(HrcDumper_ptr self, const NuSMVEnv_ptr env, FILE* fout)
{
  ...
  /* see _HRC_DUMP_COMMENT in HrcDumper_private.h */
  OVERRIDE(HrcDumper, dump_comment) = hrc_dumper_dump_comment;
  /* see _HRC_DUMP_HEADER in HrcDumper_private.h */
  OVERRIDE(HrcDumper, dump_header) = hrc_dumper_dump_header;
  /* see _HRC_DUMP_NODE in HrcDumper_private.h */
  OVERRIDE(HrcDumper, dump_node) = hrc_dumper_dump_node;
  OVERRIDE(HrcDumper, dump_var_type) = hrc_dumper_dump_var_type;
  OVERRIDE(HrcDumper, dump_annotations) = hrc_dumper_dump_annotations;
}
                             ...
                             const HrcDumperInfo* info)
{
  ...
  case NUXMV_HDS_LIST_PREDICATES:
    if (info->stage & HRC_STAGE_BEGIN) {
    }
    if (info->stage & HRC_STAGE_END) {
    }
...
void hrc_dumper_dump_annotations(const HrcDumper_ptr self)
{
  error_unreachable_code_msg("TBI");
}
...
void hrc_dumper_dump_var_type(HrcDumper_ptr self, node_ptr node)
{
  ...
  case WORDARRAY_TYPE:
    ...
    HrcDumper_dump_var_type(self, cdr(node));
    break;
