...
#include "nusmv/core/hrc/HrcFlattener_private.h"
...
static void hrc_flattener_finalize(Object_ptr object, void* dummy);
...
void HrcFlattener_flatten_hierarchy(HrcFlattener_ptr self)
{
...

  {
    const NodeMgr_ptr nodemgr =
      NODE_MGR(NuSMVEnv_get_value(ENV_OBJECT(self)->environment, ENV_NODE_MGR));
  ...
  Compile_ProcessHierarchy(ENV_OBJECT(self)->environment, self->symb_table,
                                      ...
                                      FILE* out)
{
  ...
  Compile_WriteFlattenModel(ENV_OBJECT(self)->environment, out, self->symb_table, names,
...
void HrcFlattener_allow_symb_table_update(HrcFlattener_ptr self)
{
  HRC_FLATTENER_CHECK_INSTANCE(self);

  self->allow_symb_table_update = true;
}
...
void hrc_flattener_init(HrcFlattener_ptr self, const NuSMVEnv_ptr env,
                        ...
                        SymbLayer_ptr layer)
{
  ...
  self->build_hierarchy = true;
  self->populate_symb_table = true;
  self->allow_symb_table_update = false;

  OVERRIDE(Object, finalize) = hrc_flattener_finalize;
  OVERRIDE(HrcFlattener, populate_model) = hrc_flattener_populate_model;
}
                                  ...
                                  node_ptr context)
{
  ...
  /* URGENT */
  {
    node_ptr new_urgent =
      hrc_flattener_expression_and(nodemgr,
                                   HrcNode_get_urgent_exprs_iter(node));
    new_urgent = hrc_prefix_utils_contextualize_expr(env, new_urgent, context);
    FlatHierarchy_set_urgent(self->hierarchy,
                             EXPR_OR(new_urgent,
                                     FlatHierarchy_get_urgent(self->hierarchy)));
  ...
  /* ASSIGN */
  {
    ...
    instance_assign =
      hrc_flattener_proc_assign_list(self, node,
                                     HrcNode_get_urgent_assign_exprs_iter(node),
                                     instance_assign,
                                     URGENT);
                                               ...
                                               const int assign_type)
{
  ...
  for(; ! Oiter_is_end(assign_iter); assign_iter = Oiter_next(assign_iter)) {
    ...
    case URGENT:
      assign = find_node(nodemgr, EQDEF,
                         find_node(nodemgr, URGENT, car(assign), Nil),
                         cdr(assign));
      break;
    default:
...
/*!
  \brief The HrcFlattener class virtual finalizer

  Called by the class destructor
*/
static void hrc_flattener_finalize(Object_ptr object, void* dummy)
{
  HrcFlattener_ptr self = HRC_FLATTENER(object);
