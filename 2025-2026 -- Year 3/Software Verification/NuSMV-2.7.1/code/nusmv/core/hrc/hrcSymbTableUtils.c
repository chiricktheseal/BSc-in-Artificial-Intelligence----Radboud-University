                                              ...
                                              boolean allow_update)
{
  ...
  /* PARAMETERS */
  {
    ...
    for(; ! Oiter_is_end(actuals);
      ...
      resolve_symbol = SymbTable_resolve_symbol(symb_table, formal, context);
      resolved_name = ResolveSymbol_resolve(resolve_symbol, symb_table, formal,
                                            context);

      if (! allow_update || ! SymbTable_is_symbol_parameter(symb_table, resolved_name)) {
        SymbLayer_declare_parameter(layer, form_flat, 
                                    hrc_prefix_utils_pop_context(env, context), 
                                    actual);
      }
    }
  }
  ...
  /* FUNCTIONs */
  {
    ...
    for(; ! Oiter_is_end(functions); functions = Oiter_next(functions)) {
      ...
      if (! allow_update || ! SymbTable_is_symbol_function(symb_table, name)) {
  ...
  /* DEFINEs */
  {
    ...
    for(; ! Oiter_is_end(defines); defines = Oiter_next(defines)) {
      ...
      if (! allow_update || ! SymbTable_is_symbol_define(symb_table, name)) {
  ...
  /*  ARRAY DEFINE */
  {
    ...
    for(; ! Oiter_is_end(array_defines); array_defines = Oiter_next(array_defines)) {
      ...
      if (! allow_update || ! SymbTable_is_symbol_array_define(symb_table, name)) {
  ...
  /* Constants do not need any type of flattening */
  {
    ...
    for(; ! Oiter_is_end(constants); constants = Oiter_next(constants)) {
      ...
      /* Constants may have been already declared (for example by a
         variable with symbolic enum type, or by another module) */
                                                   ...
                                                   boolean allow_update)
{
  ...
  for(; ! Oiter_is_end(vars_iter); vars_iter = Oiter_next(vars_iter)) {
    ...
    if (PROCESS == node_get_type(type)) {
      ...
      resolve_symbol = SymbTable_resolve_symbol(symb_table, var, context);
      resolved_name = ResolveSymbol_resolve(resolve_symbol, symb_table, var, context);

      opts = OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));

      if (! allow_update ||
          (! SymbTable_is_symbol_var(symb_table, resolved_name) &&
           ! SymbTable_is_symbol_variable_array(symb_table, resolved_name) &&
           ! (! opt_keep_single_value_vars(opts) &&
              SymbTable_is_symbol_define(symb_table, resolved_name)))) {
        symbolicType = Compile_InstantiateType(symb_table,
                                               layer,
                                               var, type, context);
        success = Compile_DeclareVariable(symb_table, layer,
