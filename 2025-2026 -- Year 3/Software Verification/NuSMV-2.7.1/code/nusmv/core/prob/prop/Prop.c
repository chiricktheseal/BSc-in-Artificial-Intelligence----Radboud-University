...
static Object_ptr prop_copy(const Object_ptr self);
...
/*---------------------------------------------------------------------------*/
NuSMVEnv_ptr Prop_get_env(Prop_ptr prop) {
  return ENV_OBJECT(prop)->environment;
}
...
Prop_ptr Prop_copy(const Prop_ptr self)
{
  Prop_ptr copy;

  PROP_CHECK_INSTANCE(self);

  copy = PROP(OBJECT(self)->copy(OBJECT(self)));

  PROP_CHECK_INSTANCE(copy);
...
Expr_ptr Prop_get_expr_core(const Prop_ptr self)
{
...

  {
    ...
    /* Note removed from here because bounds could be reals,
       moved in Prop_Rewriter */
    res = Compile_remove_ltl_bop(env, res);
    return Compile_pop_distrib_ops(env, res);
  }
}

Expr_ptr Prop_get_expr_core_keep_ltl_bop(const Prop_ptr self)
{
  PROP_CHECK_INSTANCE(self);
...
{
  const NuSMVEnv_ptr env = ENV_OBJECT_GET_ENV(st);
  const MasterWffRewriter_ptr rewriter =
    MasterWffRewriter_create(env);
  boolean result;

  NUSMV_ENV_CHECK_INSTANCE(env);
  MASTER_WFF_REWRITER_CHECK_INSTANCE(rewriter);
  ...
  if (Prop_Psl == self->type) {
    ...
    result =
      MasterWffRewriter_is_rewriting_needed(rewriter, st,
                                            WFF_REWRITER_REWRITE_INPUT_NEXT,
                                            Prop_get_expr(self), Nil);
  }

  MasterWffRewriter_destroy(rewriter);
...
void Prop_set_printer(FILE* file, Set_Element_t el, void* arg)
{
  OStream_ptr ostream;
  Prop_PrintFmt fmt;

  ostream = OStream_create(file);
  if (NULL == arg) {
    fmt = PROP_PRINT_FMT_DEFAULT;
  }
  else fmt = PTR_TO_INT(arg);

  Prop_print(PROP(el), ostream, fmt);
...
void prop_init(Prop_ptr self, const NuSMVEnv_ptr env)
{
  ...
  OVERRIDE(Object, copy) = prop_copy;
...
void prop_copy_aux(const Prop_ptr self,
                   Prop_ptr other)
{
  PROP_CHECK_INSTANCE(other);

  problem_copy_aux(PROBLEM(self), PROBLEM(other));

  other->index = self->index;
  other->prop = self->prop;

  /* the cone is just referenced in the property */
  other->cone = self->cone;
  other->type = self->type;
  other->status = self->status;
  other->number = self->number;
  other->trace = self->trace;
  /* Setting the fsms to NULL is necesssary to avoid the set fsm methods try
     destroy an invalid pointer */
  other->scalar_fsm = NULL;
  other->bool_fsm = NULL;
  other->bdd_fsm = NULL;
  other->be_fsm = NULL;
  prop_set_scalar_sexp_fsm(other, self->scalar_fsm, true);
  prop_set_bool_sexp_fsm(other, self->bool_fsm, true);
  prop_set_bdd_fsm(other, self->bdd_fsm, true);
  prop_set_be_fsm(other, self->be_fsm, true);
  other->name = self->name;

  other->get_expr = self->get_expr;
  other->get_type_as_string = self->get_type_as_string;
  other->print = self->print;
  other->print_truncated = self->print_truncated;
  other->print_db_tabular = self->print_db_tabular;
  other->print_db_xml = self->print_db_xml;
  other->verify = self->verify;
  other->set_environment_fsms = self->set_environment_fsms;
  other->convert_to_invar = self->convert_to_invar;
}
