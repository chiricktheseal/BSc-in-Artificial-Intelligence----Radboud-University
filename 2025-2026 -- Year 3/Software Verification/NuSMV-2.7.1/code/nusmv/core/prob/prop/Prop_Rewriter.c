...
#include "nusmv/core/prob/prop/Prop_Rewriter.h"
#include "nusmv/core/prob/prop/Prop_Rewriter_private.h"
...
Prop_Rewriter_ptr Prop_Rewriter_create_extended
                                      (NuSMVEnv_ptr env,
                                       Prop_ptr prop,
                                       WffRewriteMethod method,
                                       WffRewriterExpectedProperty expprop,
                                       ...
                                       SymbLayer_ptr layer)
{
  ...
  prop_rewriter_init(self, env, prop, method, expprop, fsm_type, bddenc, layer);

  /* Here the only chance to know if there's an external layer supplied. */
  if (SYMB_LAYER(NULL) != layer) {
    self->external_layer_p = true;
  } else {
    self->external_layer_p = false;
  }
...
void Prop_Rewriter_destroy(Prop_Rewriter_ptr self)
{
...

  {
    ...
    boolean external_layer_p = self->external_layer_p;

    /* Do not remove externally created symbol layers. */
    if (external_layer_p) { return; }
...
FlatHierarchy_ptr Prop_Rewriter_get_flat_hierarchy(const Prop_Rewriter_ptr self)
{
  PROP_REWRITER_CHECK_INSTANCE(self);

  return self->generated_fh;
}
...
Prop_ptr prop_rewriter_rewrite(Prop_Rewriter_ptr self)
{
  ...
  /* Maybe there is nothing to do */
    ...
    {
      const MasterWffRewriter_ptr rewriter = MasterWffRewriter_create(env);
      node_ptr formula = NULL;
      short int spec_type = 0;

      outfh = FlatHierarchy_create(self->symb_table);
      self->generated_fh = outfh;
      formula = Prop_get_expr_core(self->original);
      formula = Compile_remove_ltl_bop(env, formula);
      spec_type = PropType_to_node_type(Prop_get_type(self->original));

      rewritten_formula =
        MasterWffRewriter_rewrite_formula(rewriter, self->method, self->expprop,
                                          layer, outfh, formula, spec_type,
                        ...
                        SymbLayer_ptr layer)
{
  ...
  /* owned */
  self->rewritten = NULL;
  self->generated_fh = NULL;

  /* default value of the extended slot */
  self->external_layer_p = false;
...
void prop_rewriter_deinit(Prop_Rewriter_ptr self)
{
  ...
  /* if the rewriting is not applied the hierarchy is NULL */
  if (NULL != self->generated_fh) {
    FlatHierarchy_destroy(self->generated_fh); self->generated_fh = NULL;
  }
