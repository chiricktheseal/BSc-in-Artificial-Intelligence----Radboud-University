  ...
  {  /* To evaluate a set of expressions, we first set context for
    ...
    TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_S_DEFINES, sym_iter, sym) {
      if (Nil != Trace_step_get_value(trace, step, sym)) {
        ...
        if ((Nil != val) && (FAILURE != node_get_type(val))) {
...
{
  trace_step_evaluate_define_list(trace, step, NODE_LIST(NULL));
}

void trace_step_evaluate_define_list(Trace_ptr trace,
                                     ...
                                     const NodeList_ptr define_list)
{
  ...
  {  /* To evaluate a set of expressions, we first set context for
    ...
    /* 1 . state defines */
    TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_S_DEFINES, sym_iter, sym) {
      /* if list is NULL evaluate all state defines */
      if (NULL == define_list || NodeList_belongs_to(define_list, sym)) {
        if (Nil == Trace_step_get_value(trace, step, sym)) {
          node_ptr def_body = SymbTable_get_define_flatten_body(st, sym);
          val = BaseEvaluator_evaluate(evaluator, def_body);
    ...
    { /* 2. transitional defines (exist only if there's next state) */
      ...
      if (TRACE_END_ITER != next) {
        TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_TRANSITIONAL, sym_iter, sym) {
          /* if list is NULL evaluate all transitional defines */
          if (NULL == define_list || NodeList_belongs_to(define_list, sym)) {
            if (Nil == Trace_step_get_value(trace, next, sym)) {
              node_ptr def_body = SymbTable_get_define_flatten_body(st, sym);
              val = BaseEvaluator_evaluate(evaluator, def_body);
...
void Trace_Eval_evaluate_define_list(Trace_ptr trace,
                                     const NodeList_ptr define_list)
{
  TraceIter step;

  TRACE_FOREACH(trace, step) {
    trace_step_evaluate_define_list(trace, step, define_list);
  }
}
/*---------------------------------------------------------------------------*/
...
static hash_ptr trace_eval_make_environment(Trace_ptr trace, TraceIter step)
{
  ...
  TRACE_SYMBOLS_FOREACH(trace, TRACE_ITER_F_VARS, sym_iter, sym) {
    const node_ptr val = Trace_step_get_value(trace, step, sym);
    insert_assoc(res, sym, val);
    insert_assoc(res, find_node(nodemgr, NEXT, sym, Nil), val);
  }
  ...
  /* 4. Insert rigid function values */
    /* Dump of the rigid functions */
  {
    NodeList_ptr rigidfunctions = Trace_rigidfunctions_get_values(trace);
    ListIter_ptr rit;

    if (0 < NodeList_get_length(rigidfunctions)) {
      NODE_LIST_FOREACH(rigidfunctions, rit) {
        node_ptr e = NodeList_get_elem_at(rigidfunctions, rit);
        node_ptr s = car(e);
        node_ptr v = cdr(e);
        insert_assoc(res, s, v);
      }
    }
    NodeList_destroy(rigidfunctions);
  }
