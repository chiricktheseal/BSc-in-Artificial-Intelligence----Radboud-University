...
typedef struct Trace_TAG
{
  ...
  array_t * rigidfunctions_frame_values;
  array_t * rigidfunctions_frame_names;
  hash_ptr rigidfunctions_frame_name_has_value;
  ...
  /* virtual functions */
  VIRTUAL bool (*equals)(const Trace_ptr self, const Trace_ptr other);
  VIRTUAL TraceVarFrame_ptr (*create_var_frame)(const Trace_ptr self);
  VIRTUAL boolean (*is_complete_vars)(const Trace_ptr self,
...
*/
void trace_init(Trace_ptr self, SymbTable_ptr st, const short kind, const char* desc,
                const TraceType type, NodeList_ptr symbols,
                boolean is_volatile, boolean allow_bits);

/*!
  \brief returns trace kind.

  Trace kind is used to disambiguate between the different subclasses.
  A value of `TRACE_BASE_KIND` represents the base class.

  WARNING: consistency of such ids is responsibility of the programmers,
           when creating a subclass of trace please check the ids used by other
           subclasses to avoid collisions.

  kind is used to detect the type of a trace,
  e.g. a plugin might check if the kind of the trace is greater of equal
       to the kind of traces it is able to handle. The assumption is that
       subtypes have kind greater with respect to the one of the base class.
       Subtypes that belong to another inheritance path should not reach that plugin.
*/
short trace_get_kind(const Trace_ptr self);


/*!
  \brief initialize initial frame of self to the given initial_frame

  \se creates a define and a frozen frame and adds them to self.
*/
void trace_init_initial_frame(Trace_ptr self, TraceVarFrame_ptr initial_frame);
...
*/
void
trace_copy(Trace_ptr self, Trace_ptr other, TraceIter until_here,
           boolean is_volatile);
...
/*!
  \methodof Trace
  \brief free internal resources of self, does not free itself.

*/
void trace_deinit(Trace_ptr self);

/*!
  \methodof Trace
  \brief free resources allocated by object

  object must be a Trace instance, user should not call these method,
  call Trace_destroy instead.
*/
void trace_finalize(Object_ptr object, void* dummy);
...
/*!
  ...
  \todo Missing description
*/
boolean trace_rigidfunctions_put_value(Trace_ptr self,
...
/*!
  ...
  \todo Missing description
*/
NodeList_ptr trace_rigidfunctions_get_values(Trace_ptr self);
...
/*!
  \methodof Trace
  \brief base implementation of virtual method Trace_is_complete

  return True iff self is complete.
*/
boolean
...
/*!
  \methodof Trace
  \brief Evaluates defines contained in `define_list` for a trace

  Evaluates define for a trace, based on assignments to
               state, frozen and input variables.

  If `define_list` is NULL. All the defines get evaluated.
  \se The trace is filled with defines
*/
void
trace_step_evaluate_define_list(Trace_ptr self,
                                const TraceIter step,
                                const NodeList_ptr define_list);
/*!
  \methodof Trace
  \brief
...
/* methods to deal with Frames,
   TODO create separate files for Frames */

/*!
  \brief initialize given frame in order to be added to the given trace.

  \se modifies self
*/
void trace_var_frame_init(TraceVarFrame_ptr self, Trace_ptr trace);

void trace_var_frame_deinit(TraceVarFrame_ptr self);

void trace_var_frame_destroy(TraceVarFrame_ptr self);
