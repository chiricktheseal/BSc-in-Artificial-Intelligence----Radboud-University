...
/*!
  \methodof Trace
  \brief Stores a value for a rigid function
...
*/
boolean
Trace_rigidfunctions_put_value(Trace_ptr self,
                               node_ptr symb, node_ptr value);


/*!
  \methodof Trace
  \brief Gets a list of values for rigid functions

  Returns a list of f(...) = v.

*/
NodeList_ptr
Trace_rigidfunctions_get_values(Trace_ptr self);
...
/*!
  \methodof Trace
  \brief return string representation of the given trace type.
...
/*!
  \methodof Trace
  \brief Returns the count of the loopbacks of the Trace

  Iterate through the various trace steps and count the number of steps which
  are loopback steps.

  \se None
*/
int Trace_get_loopback_count(const Trace_ptr self);
...
/*!
  \methodof Trace
  \brief Returns the formula in a form of conjunctions of funtions

  Gets functions from initial state, since the denotation of the
  function is defined in the initial state and kept from that point on

  \se None
*/
Expr_ptr Trace_get_initial_functions_formula(const NuSMVEnv_ptr env,
                                             const Trace_ptr trace);

/*!
  \methodof Trace
  \brief Returns the formula in a form of conjunctions of frozenvars

  Gets frozenvars from initial state, since frozenvars retain their initial
  value throughout the evolution of the state machine

  \se None
*/
Expr_ptr Trace_get_initial_frozenvars_formula(const NuSMVEnv_ptr env,
                                              const Trace_ptr trace);
