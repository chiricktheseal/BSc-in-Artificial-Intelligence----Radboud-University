...
static int CommandCompassGenSigref(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  while ((c = Utils_getopt(&opt, argc, argv, "hbi:t:o:a:")) != EOF) {
    switch (c) {
    case 'b':
      do_indent = true;
      break;
    ...
    case 'i':
      ...
      prob_fname = util_strsav(opt.optarg);
      break;

    case 't':
      if (tau_expr_str != (char*) NULL) { FREE(tau_expr_str); }
      tau_expr_str = util_strsav(opt.optarg);
      break;

    case 'o':
      if (sigref_fname != (char*) NULL) FREE(sigref_fname);
      sigref_fname = util_strsav(opt.optarg);
      break;

    case 'a':
      if (ap_fname != (char*) NULL) FREE(ap_fname);
      ap_fname = util_strsav(opt.optarg);
      break;
  ...
  if (argc != opt.optind) goto __compass_gen_sigref_fail_help;
