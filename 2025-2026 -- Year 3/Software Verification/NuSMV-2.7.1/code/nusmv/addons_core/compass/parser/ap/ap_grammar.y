...
%type <node> concatenation_expr multiplicative_expr
...
concatenation_expr :
               primary_expr
             | concatenation_expr TOK_CONCATENATION primary_expr { $$ = new_node(NODEMGR, CONCATENATION, $1, $3); }
             ;

multiplicative_expr :
               concatenation_expr
             | multiplicative_expr TOK_TIMES concatenation_expr  { $$ = new_node(NODEMGR, TIMES, $1, $3); }
             | multiplicative_expr TOK_DIVIDE concatenation_expr { $$ = new_node(NODEMGR, DIVIDE, $1, $3); }
             | multiplicative_expr TOK_MOD concatenation_expr    { $$ = new_node(NODEMGR, MOD, $1, $3); }
             ;
