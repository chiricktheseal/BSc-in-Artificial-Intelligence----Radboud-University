...
def generate_test(
    ...
    for arg_type in constructor[1]:
        constr_args.append(str(generate_value_for_type(arg_type)))
        
    #create object + use generetaed arguemtns    
    test_lines = []
    obj_name = f"obj_{random.randint(0, 999)}"
    ...
    if other_methods:
        num_methods = min(5, len(other_methods))
        ...
        for i, (method, arg_types, return_type) in enumerate(selected_methods):
            ...
            for arg_type in arg_types:
                if arg_type in classes and arg_type != class_name:
                    other_class_methods = classes[arg_type]
                    other_constructor = None
                    for m in other_class_methods:
                        if m[0] == '__init__':
                            other_constructor = m
                            break
                    if other_constructor:
                        other_args = []
                        for at in other_constructor[1]:
                            other_args.append(str(generate_value_for_type(at)))
                        other_obj_name = f"{arg_type.lower()}_{random.randint(0, 999)}"
                        test_lines.append(f"{other_obj_name} = {arg_type}({', '.join(other_args)})")
                        method_args.append(other_obj_name)
                    else:
                        method_args.append('None')
                else:
                    method_args.append(str(generate_value_for_type(arg_type)))
            
            test_lines.append(f"var{i} = {obj_name}.{method}({', '.join(method_args)})")
...
def generate_value_for_type(type_str):
    if type_str == 'bool':
        ...
        # Для tuple[float, float] или tuple[int, int]
