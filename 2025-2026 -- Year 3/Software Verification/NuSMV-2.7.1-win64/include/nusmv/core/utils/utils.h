...
*/
boolean Utils_fcopy(FILE* input,
                    FILE* copy);
...
Utils_getopt_state Utils_getopt_init(void);
int Utils_getopt(Utils_getopt_state *state,
                 int argc, char *argv[], char *optstring);

/*!
  \brief Parse an Olist into a cons'd list

  Assumes that `list` contains node pointers.
  The returned list should be destroyed.
*/ 
node_ptr Utils_olist_to_nodelist(const NodeMgr_ptr nodemgr,
                                 const Olist_ptr list);

/*!
  \brief Parse a cons'd list into a new Olist

*/
Olist_ptr Utils_nodelist_to_olist(node_ptr nodelist);

/*!
  \brief Split a string based on a delimiter
*/
Olist_ptr Utils_split_string(char *s, const char *delimiter);
...
*/
string_ptr Utils_generate_string(UStringMgr_ptr strings, const char* format, ...);

/*!
  \brief Convert the whole input string to upper cases. The input string is
         modified (and then returned as is.)

  \sa _strupr() in <string.h> (MSVC only)
*/
char* Utils_strupr(char* string);
