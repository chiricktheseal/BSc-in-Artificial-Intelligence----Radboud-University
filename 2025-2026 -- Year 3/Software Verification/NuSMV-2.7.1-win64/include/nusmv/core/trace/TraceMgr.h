...
/*!
  \methodof TraceMgr
  \brief Replaces the plugin stored at given index.
  WARNING: does not destroy the old plugin
*/
void
TraceMgr_set_plugin_at_index(const TraceMgr_ptr self, int index,
...
/*!
  \methodof TraceMgr
  \brief Removed from env the default plugins.

  Removes from env the available plugins
*/
void TraceMgr_quit_plugins(TraceMgr_ptr self);
