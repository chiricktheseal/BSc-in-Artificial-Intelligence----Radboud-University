...
/*!
  \brief Macro for iteration with additional filter.

  Note: module symb_table_filter contains a few pre-defined functions
  suitable for being used as filters.
*/
...
/* When "always_create" is true, create the layer with "layer_name+suffix" */
...
/*!
  \methodof SymbTable
  \brief Checks if an SMT solver is needed

  Actually, it checks if an SMT solver is needed. For this reason, it returns
  true also if functions are present
*/
boolean
...
/*!
  \methodof SymbTable
  \brief Checks whether the Symbol Table contains continuous variables

  Checks whether the Symbol Table contains continuous variables
*/
boolean
...
/*!
  \methodof SymbTable
  \brief Checks whether the Symbol Table contains clock variables

  Checks whether the Symbol Table contains clock variables
*/
boolean
...
/*!
  \methodof SymbTable
  \brief True if all the symbols in self occur in other

  All the symbols in self must occur in other and have the same type
*/
...
/*!
  \methodof SymbTable
  \brief marks the given variable as a non continuous variable
...
/*!
  \methodof SymbTable
  \brief removes the given variable from the non continuous ones.

  the given variable in unmarked and it is no more non continuous.
  \sa SymbTable_mark_var_as_non_continuous
  \se modifes self
*/
...
/*!
  \methodof SymbTable
  \brief removes all variables from the non continuous ones.

  all noncontinuous variable in the symbol table are unmarked.
  \sa SymbTable_mark_var_as_non_continuous, SymbTable_unmark_non_continuous_var.
  \se modifes self
*/
...
/*!
  \methodof SymbTable
  \brief return true iff the given variable is marked as non continuous.
