...
/*!
  \methodof TypeChecker
  \brief TypeChecker copy constructor

  TypeChecker class copy constructor.  As the copy may refer to a
  different symbol table than the original, the symbol table of the
  copy is taken.  Notice that Object_copy will not set a different ST,
  but will copy also the symbol table which is referenced by the
  original.

  Important: self's environment and given st must be sharing the same
  environment.

  \sa TypeChecker_create
*/
TypeChecker_ptr
TypeChecker_copy(const TypeChecker_ptr self, struct SymbTable_TAG* st);
...
*/
boolean
TypeChecker_check_constraints(TypeChecker_ptr self,
                              node_ptr init, node_ptr trans,
                              node_ptr invar, node_ptr assign, node_ptr urgent,
...
/*!
  \methodOf TypeChecker
  \brief returns a sub-expression of the given expression of type `type`.

  returns a sub-expression of the given expression of type `type`.
