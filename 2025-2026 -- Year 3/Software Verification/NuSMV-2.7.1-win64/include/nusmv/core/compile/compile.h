...
/*!
  \brief Pop context from ctx
  
  Opposite function of CompileFlatten_concat_contexts,

  given a context, removes the right side of the context.
  "a.b.c" becomes "a.b", 
  expressions like "a.b[1][2][3][4][5][6]" became "a",
  in case of an ATOM, a NUMBER or Nil returns Nil

  Important: normalize returned node.

  \sa CompileFlatten_concat_contexts
*/
node_ptr
CompileFlatten_pop_context(const NuSMVEnv_ptr env,
                           node_ptr ctx);
/*!
...
/*!
  \brief Return the left index of an internal array

  node must be an INTERNAL_ARRAY_STRUCT
*/
int Compile_get_internal_array_lower_bound(node_ptr node);

/*!
  \brief Return the right index of an internal array

  node must be an INTERNAL_ARRAY_STRUCT
*/
int Compile_get_internal_array_upper_bound(node_ptr node);

/*!
  \brief Return the subtype of an internal array

  node must be an INTERNAL_ARRAY_STRUCT
*/
SymbType_ptr Compile_get_internal_array_subtype(node_ptr node);

/*!
  \brief Return the local array of an internal array

  node must be an INTERNAL_ARRAY_STRUCT
*/
node_ptr* Compile_get_internal_array_local_array(node_ptr node);


/*!
  \brief Return the right index of the local array of an internal array

  Useful when iterating.

  node must be an INTERNAL_ARRAY_STRUCT
*/
int Compile_get_internal_array_local_array_size(node_ptr node);
