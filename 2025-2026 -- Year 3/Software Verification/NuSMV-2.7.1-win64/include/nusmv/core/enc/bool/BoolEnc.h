...
/*!
  \methodof BoolEnc
  \brief Traverse the bit encoding of a scalar var, doing user-defined actions.

  The helper function is called in the following ways:

  - when the traversing reaches a constant symbol (including numbers),
    "helper(self, 0, bit, data)" is called.

  - at IFTHENELSE node, before going into its left (then) branch,
    "helper(self, 1, bit, data)" is called.

  - at IFTHENELSE node, after returned from left (then) branch,
    before going into its right (then) branch,
    "helper(self, 2, bit, data)" is called.

  - at UNSIGNED_WORD node, for each bit of the word,
    "helper(self, 3, bit, data)" is called.
 */
void
BoolEnc_traverse_encoding(const BoolEnc_ptr self, node_ptr enc,
                          void (*helper)(BoolEnc_ptr, int, node_ptr, void*),
                          void* data);
