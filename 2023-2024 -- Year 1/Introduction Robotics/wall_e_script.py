...
def get_center_region(image_data, region_size=8):
...
def avoid_wall():
    ...
    if sonar_value < sonarThreshold:
        object_color = detect_object_color()
...
# battery monitor
def monitor_battery(sensor_data):
    try:
        # b'0.99994973689502'
...
def detect_wall_color():
    ...
    center_top_image_data = get_center_region(top_image_data)
...
def detect_object_color():
    ...
    center_small_image_data = get_center_region(small_image_data)
...
def find_block():
    ...
    while True:
        ...
        block_color = detect_object_color()
...
while True:
    try:
        ...
        detect_wall_color()
        detect_object_color()
        #monitor_battery(top_image_data)
        find_block()
